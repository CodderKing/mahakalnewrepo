...
function getReferralCodeFromURL() {
  ...
  if (getAuthStatus == 0) {
    ...
    const referralCodeParameter = new URLSearchParams(currentUrl.search).get(
      "referral_code"
    );
...
$(".get-view-by-onclick-remove").on("click", function () {
  location.href = $(this).data("link");
});
...
function sortShopProductsByData(value) {
  $.get({
    ...
    data: {
      ...
      sub_sub_category_id: $("#store-request-data-sub-sub-category-id").data(
        "value"
      ),
...
// Pooja Code Apply
function poojacouponCode() {
  $("#pooja-coupon-code").on("click", function () {
    $.ajaxSetup({
      headers: {
        "X-CSRF-TOKEN": $('meta[name="_token"]').attr("content"),
      },
    ...
    $.ajax({
      type: "POST",
      url: $("#route-coupon-pooja").data("url"),
      data: $("#coupon-code-pooja-ajax").serializeArray(),
      success: function (data) {
        let messages = data.messages;
        if (data.status) {
          messages.forEach(function (message, index) {
    ...
    success: function (data) {
      ...
      if (parseInt(countWishlistElement.html()) % 15 === 0) {
        ...
        if (wishlistPaginatedPage.val() == 1) {
          ...
          window.location.href =
            $("#route-wishlists").data("url") + "?page=" + (pageValue - 1);
        }
      }
    },
    complete: function () {
  ...
  $(".btn-number").click(function (e) {
    ...
    if (!isNaN(currentVal)) {
      if (type == "minus") {
        ...
        if (
          parseInt($(".input-number").val()) == $(".input-number").attr("min")
        ) {
  ...
  $(".input-number").change(function () {
    ...
    if (valueCurrent >= minValue) {
      ...
      Swal.fire({
        ...
        text: $("#message-sorry-the-minimum-order-quantity-not-match").data(
          "text"
        ),
      });
...
function updateCartQuantity(cart_id, product_id, action, event) {
  ...
  if (cart_quantity_of.val() == 0) {
    ...
    if (cart_quantity_of.val() < cart_quantity_of.data("min")) {
      ...
      toastr.error(
        $("#message-minimum-order-quantity-cannot-less-than").data("text") +
          " " +
          min_value
      );
        ...
        function (response) {
          ...
          if (response["qty"] == cart_quantity_of.data("min")) {
            cart_quantity_of
              ...
              .html('<i class="tio-delete-outlined text-danger fs-10"></i>');
          } else {
            cart_quantity_of
              .parent()
              .find(".quantity__minus")
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (
          ...
          $(".btn-number[data-type='minus'][data-field='quantity']").removeAttr(
            "disabled"
          );
        } else {
          $(".btn-number[data-type='minus'][data-field='quantity']").attr(
            "disabled",
            true
          );
        }
