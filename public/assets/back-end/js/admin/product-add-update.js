...
function getDigitalProductTypeFunctionality() {
  ...
  if (digitalProductType && digitalProductType.toString() === "ready_product") {
    ...
    digitalProductType &&
    digitalProductType.toString() === "ready_after_sell"
  ) {
    $("#digital_file_ready_show").hide();
    $("#digital_file_ready").val("");
  }
...
$("#choice_attributes").on("change", function () {
  ...
  $.each($("#choice_attributes option:not(:selected)"), function () {
    let selectedValue = $(this).val();
    let selectedText = $(this).text();
    let className = `${selectedText.replace(/[^a-zA-Z0-9]/g, '')}${selectedValue.replace(/[^a-zA-Z0-9]/g, '')}`;
    $('#customer_choice_options').find(`div.${className}`).remove();
  });
...
function addMoreCustomerChoiceOption(index, name) {
  ...
  if ($('#customer_choice_options').find(`[name="choice_no[]"][value="${index}"]`).length === 0) {
      let genHtml = `<div class="col-md-6 ${name.replace(/[^a-zA-Z0-9]/g, '')}${index.replace(/[^a-zA-Z0-9]/g, '')}"><div class="form-group">
                  ...
                  <div class="">
                      <input type="text" class="form-control option_value_new${index}" name="choice_options_${index}[]" placeholder="` + messageEnterChoiceValues + `" data-role="tagsinput" onchange="getUpdateSKUFunctionality()">
                  </div>
              </div>
      </div>`;
    ...
    if (result.value) {
      ...
      $(".variation-price-input").each(function () {
        ...
        if (variationPrice < discountValue) {
          toastr.error(
            $("#message-discount-will-not-larger-then-variant-price").data(
              "text"
            )
          );
          submitStatus = 0;
        ...
        $.post({
          ...
          success: function (data) {
            if (data.errors) {
              ...
              toastr.success(
                $("#message-product-added-successfully").data("text"),
                {
                  CloseButton: true,
                  ProgressBar: true,
                }
