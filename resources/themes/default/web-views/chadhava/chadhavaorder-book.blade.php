...
@section('title', $chadhavaLeads->chadhava_name)
@push('css_or_js')
                    ...
                    <div class="cards mb-3" style="border-radius: 10px;border: 1px solid #e4eef9;">
                        <div class="card-header" id="">
                            <div class="details __h-100">
                                <span class="mb-2 __inline-24">{{ $chadhavaLeads->chadhava->name }}</span>
                                {{-- <div class="d-flex justify-content-between"> {{ $leadsGet->package_name }}

                                    <span class=""><b>{{ $leadsGet['package_price'] }} </b></span>
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <span>{{ $chadhavaLeads->chadhava->chadhava_venue }}</span>
                                    </div>
                                    ...
                                    <div class="flex items-center space-x-2 mt-2">
                                        <img src="{{ theme_asset(path: 'public\assets\front-end\img\track-order\date.png') }}"
                                            alt="" style="width:24px;height:24px;">
                                        <span>
                                            {{ date('d', strtotime($chadhavaLeads->booking_date)) }},
                                            {{ translate(date('F', strtotime($chadhavaLeads->booking_date))) }} ,
                                            {{ translate(date('l', strtotime($chadhavaLeads->booking_date))) }}
                                        </span>
                                    </div>
            ...
            <div class="row g-3 mx-max-md-0">
                                            ...
                                            <td class='__w-15p text-center'>
                                                <div class='qty d-flex justify-content-center align-items-center gap-3'>
                                                    ...
                                                    <input type='text'
                                                        ...
                                                        data-increment="{{ '1' }}"
                                                        oninput='QuantityUpdate("{{ $pval->product_id }}","{{ $pval->id }}" this.value)' readonly disabled>
                        ...
                        <div class="__cart-total __cart-total_sticky">
                            <div class="cart_total p-0">
                                ...
                                @php
                                    ...
                                    $totalAmount = $final_price_val - $customer->wallet_balance;
                                @endphp
                                    ...
                                    <div id="productCounts">
                                        <div class="finalProducts">
                                            ...
                                            <div class="d-flex justify-content-between">
                                                ...
                                                @if ($customer->wallet_balance < $final_price_val)
                                                    <span class="cart_value text-danger"> -
                                                        {{ webCurrencyConverter(amount: $customer->wallet_balance) }}</span>
                                                @else
                                                    <span class="cart_value text-danger" id="mainProductPrice"> -
                                                        {{ webCurrencyConverter(amount: $final_price_val) }}</span>
                                                @endif
                                            </div>
                                            @if ($customer->wallet_balance < $final_price_val)
                                                <div class="d-flex justify-content-between">
                                ...
                                <div class="d-flex justify-content-between">
                                    ...
                                    <span class="cart_value" id="mainProductPrice">
                                        @if ($customer->wallet_balance < $final_price_val)
                                            {{ webCurrencyConverter($final_price_val - $customer->wallet_balance) }}
                                        @else
                            ...
                            @if ($digital_payment['status'] == 1)
                                @foreach ($payment_gateways_list as $payment_gateway)
                                    <form method="post" class="digital_payment" id="{{ $payment_gateway->key_name }}_form"
                                        action="{{ route('customer.chadhava-payment-request') }}">
                                        @csrf
                                        <div class="Details">
                                            ...
                                            <input type="hidden" name="external_redirect_link"
                                                value="{{ url('/') . '/chadhava-web-payment' }}">
                                            ...
                                            <input type="hidden" name="wallet_balance"
                                                value="{{ $customer->wallet_balance < $final_price_val ? $customer->wallet_balance : $final_price_val }}">
                                            <input type="hidden" name="booking_date"
                                                value="{{ $chadhavaLeads->booking_date }}">
                                            <input type="hidden" name="service_id" value="{{ $chadhavaLeads->id }}">
                                            ...
                                            <input type="hidden" name="final_amount" id="Amount"
                                                value="{{ $final_price_val }}">
                                            @php
                                                $final_price_val - $customer->wallet_balance;
                                            @endphp
                                            @if ($customer->wallet_balance < $final_price_val)
                                            @else
                                            ...
                                            @if ($customer->wallet_balance < $final_price_val)
                                                <input type="hidden" name="payment_amount" id="mainProductPriceInput"
                                                    value="{{ $final_price_val - $customer->wallet_balance }}">
                                            @else
                                                <input type="hidden" name="payment_amount" id="mainProductPriceInput"
                                                    value="{{ 0.0 }}">
                                            @endif
                                        ...
                                        <div class="mt-4 d-none d-sm-block">
                                            <button type="submit" class="btn btn--primary btn-block" data-id="{{ $leadsGet->id }}" data-name="{{ $chadhavaLeads->chadhava->chadhava_venue }}"
                                                data-price="{{ $final_price_val }}" id="ChadhavaPricePayment"
                                                {{ $final_price_val ? '' : 'disabled' }}>{{ translate('Proceed_to_Checkout') }}</button>
                                        </div>
                                    </form>
                                @endforeach
                            @endif
                ...
                <section class="col-lg-6 order-572-1 order-lg-2" id="addmore">
                    @php
                        ...
                        $products_data = is_array($productIds) ? \App\Models\Product::whereIn('id', $productIds)->where('status',1)->get()
                            : collect();
                    @endphp
                    <div class="pt-3 pb-3">
                        <span class=" __text-16px font-bold text-capitalize">
                            {{ translate('List_of_all_availabel_offerings_in_this_temple') }}
                        </span>
                    </div>
                            ...
                            @foreach ($products_data as $product)
                                @if (!in_array($product->id, $selected_product_array))
                                    ...
                                    <div class="flash_deal_product rtl cursor-pointer mb-2"
                                        ...
                                        data-Pprice="{{ $product->unit_price }}">
                                        <div class="d-flex">
                                            ...
                                            <div class="flash_deal_product_details pl-3 pr-3 pr-1 d-flex align-items-center">
                                                <div>
                                                    ...
                                                    <div
                                                        ...
                                                        <p>{!! Str::limit($product->details, 80) !!}</p>
                                                    </div>
                                                    ...
                                                    <div class="d-flex flex-wrap gap-8 align-items-center row-gap-0">
                                                        ...
                                                        <button class="btn btn--primary rounded-pill text-uppercase py-1 fs-12"
                                                            ...
                                                            data-serviceid="{{ $leadsGet['service_id'] }}"> <i  class="tio-add"></i> {{ translate('add') }} </button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    <div class="button-sticky bg-white d-sm-none">
        <div class="d-flex flex-column gap-1 p-3" id="checkbutton">
            <div class="d-flex gap-3 justify-content-center" role="button">
                <button type="button" id="checkout-btn" class="bottom-btn btn btn--primary d-flex justify-content-between align-items-center w-100 px-3 py-2 submit-btn"
                    data-id="{{ $leadsGet->id }}" data-name="{{ $leadsGet->service_name }}"
                    data-price="{{ $leadsGet['package_name'] }}" onclick="submitCheckoutForm(this)" {{ $final_price_val ? '' : 'disabled' }}>
                        ...
                        <div id="package-details">
                            ...
                            <div class="mb-1 final-price" id="mainProductPrice">
                                @if ($customer->wallet_balance < $final_price_val)
                                    {{ webCurrencyConverter($final_price_val - $customer->wallet_balance) }}
                                @else
                                   {{ webCurrencyConverter(amount: $final_price_val) }}
                                @endif
                            </div>

                            <div class=" text-truncate" id="package-name">
                            {{ $chadhavaLeads->chadhava->name}}
...
@push('script')
    ...
    <script>
        function QuantityUpdate(cartId, quantity, updateid, pprice, leadid, poojaprice) {
            ...
            if (quantity == -1) {
                if (currentQty === 1) {
                    deleteQuantity(updateid, cartId, pprice, leadid);
                    return;
                } else if (currentQty === 2) {
                    $('#DeleteIcon' + cartId).addClass('tio-delete text-danger fs-12');
                    $('#DeleteIcon' + cartId).removeClass('tio-remove');
                }
                var newQuantity = currentQty + quantity;
                inputBox.val(newQuantity);
                ProductQuantity(cartId, quantity, updateid, pprice, leadid, poojaprice, newQuantity);
            } else {
                $('#DeleteIcon' + cartId).removeClass('tio-delete text-danger fs-12');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        if ($('#productCountFinal' + cartId).length) {
                            $('#productCountFinal' + cartId).val(response.data.final_price);
                        }
                        var updatedAmount = parseInt(response.data.total_amount);
                        var finalTotal = updatedAmount + parseInt(poojaprice);
                        $('#mainProductPriceInput').val(finalTotal);
                        $('#mainProductPrice').text(finalTotal + '.00');
                        $('#mainProductPriceInput').text(finalTotal + '.00');
                        $("#cart-summary").load(location.href + " #cart-summary > *");
                        $("#checkbutton").load(location.href + " #checkbutton > *");
                    } else {
                        console.warn(response.message || "Update failed");
                    }
                },
                error: function(xhr) {
        ...
        function deleteQuantity(updateid, cartId, pprice, leadid) {
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        console.warn(response.message || "Unable to delete item.");
                    }
                },
                error: function(xhr) {
