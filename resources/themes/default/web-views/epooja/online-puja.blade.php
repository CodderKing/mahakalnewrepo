                ...
                <?php
                    ...
                    <ul id="filters" class="clearfix pooja-menu" style="padding: 10px;">
                        ...
                        @foreach ($subcategory as $item)
                            <li><span class="filter" data-filter=".{{ $item->slug }}">{{ ucwords($item->name) }}</span></li>
                        @endforeach
                    </ul>

                <div id="search-bar-container" class="pooja-search" style="display: none;">
                    <div class="d-flex align-items-center gap-2">
                        <input class="form-control" type="search" autocomplete="off" placeholder="Search Pooja Name,Place,Day,Date" name="name" value="">
                    </div>
                </div>
            ...
            filterList.init();
                ...
                $('input[type="search"]').on('keyup', function() {
                    ...
                    $('#portfoliolist .portfolio').each(function() {
                        var matchesSearch = $(this).find('.puja-title').text().toLowerCase().indexOf(
                            searchText) > -1;
                        var matchesCategory = activeCategory === '{{ $dataFilterString }}' || $(this).hasClass(
