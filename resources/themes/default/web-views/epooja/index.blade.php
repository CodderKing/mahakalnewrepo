...
@endphp
@push('css_or_js')
    <meta name="description" content="{{ $epooja->meta_description }}">
...
@section('content')
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6 col-md-8 col-12 mt-md-0 mt-sm-3 web-direction" style="height: fit-content;">
                            <div class="details __h-100">
                                        ...
                                        @if (!empty($date))
                                            <div class="d-flex">
                                                ...
                                                <p class="pooja-calendar">
                                                    ...
                                                    {{ translate(date('F', strtotime($date))) }},
                                                    {{ translate(date('l', strtotime($date))) }}
                                                </p>
                                            </div>
                                        @else
                                            <div class="d-flex">
                                                <img src="{{ theme_asset(path: 'public/assets/front-end/img/track-order/date.png') }}"
                                                    alt="Booking Date" style="width:24px;height:24px;">
                                                <p class="pooja-calendar">{{ translate('No upcoming booking') }}</p>
                                            </div>
                                        @endif
                                    </div>
                                ...
                                <div class="flex flex-col">
                                    <div class="mt-2">
                                        <strong>{{ translate('Puja_Booking_will_be_closed') }}:
                                            @if ($date)
                                    ...
                                    <div class="flex flex-wrap justify-center items-center">
                                        <div class="w-70 d-flex justify-content-between">
                                            ...
                                            @foreach ($servicesGet as $service)
                                                @php
                                                    $avgRating =
                                                        !empty($service->review_avg_rating) &&
                                                        $service->review_avg_rating > 0
                                ...
                                <div class="row mt-2 p-2 partial-pooja">
                                    <div class="col-lg-12 col-md-12">
                                        <div class="mx-auto max-w-screen-sm text-center mb-4">
                                            <h2 class="text-2xl font-bold tracking-tight mt-2">Select Your Puja Package
                                            </h2>
                                            <span
                                                class="text-base font-normal">{{ translate('Choose the most suitable package for your puja. Each package is designed to meet your spiritual needs with clear pricing and included services.') }}</span>
                                        </div>
                                    </div>
                                    <div class="portfolio  d-none d-sm-block d-md-flex">
                                        @php
                                    ...
                                    {{-- Mobile View Package List --}}
                                    <div class="package-grid  d-lg-none" id="packageGrid">

                                        @foreach ($packageIds as $key => $pac)
                                            @include('web-views.epooja.partials.package-mobile', [
                                                'package' => $pac,
                                            ])
                                        @endforeach
                                    </div>
                            ...
                            <div class="w-full mr-9 px-0 pt-2">
                                <form class="needs-validation_" id="QueryForm"
                                    ...
                                    <input type="hidden" name="booking_date" id="poojaBook"
                                        value="{{ date('Y-m-d', strtotime($date)) }}" placeholder="Booking Date">
    ...
    <input type="hidden" id="fullDate" value="{{ date('Y-m-d', strtotime($date . ' -1 day')) }}">
    <input type="hidden" id="fullTime" value="{{ date('H:i:s', strtotime($epooja->pooja_time)) }} ?? 23:59:59">
...
@push('script')
        ...
        $(document).ready(function() {
            ...
            const countdown = setInterval(() => {
                ...
                if (diff <= 0) {
                    ...
                    setTimeout(() => {
                        window.location.href = "{{ route('all-puja') }}";
                    }, 2000);
                    return;
                }
    ...
    <script>
        ...
        $('#send-otp-btn').click(function(e) {
            ...
            if (checkPhoneNumbers.includes(phoneNumber)) {
        ...
        $('#verify-otp-btn').click(function(e) {
            ...
            var otp = $('#otp1').val() + $('#otp2').val() + $('#otp3').val() + $('#otp4').val() + $('#otp5').val() +
                $('#otp6').val();
            var phoneNumber1 = $('#phone-code').val();
            if (checkPhoneNumbers.includes(phoneNumber1)) {
                if (otp == 123456) {
    ...
    <script>
        $(document).ready(function() {
            function selectCard(el) {
                ...
                $('.package-card').removeClass('active-card');
                $('.package-card .select-circle').css('background', 'white');
                ...
                el.find('.select-circle').css('background', color);

                // Update hidden form fields
                $('#selectedPackageId').val(id);
                $('#selectedPackagePrice').val(price);
                $('#selectedPackageName').val(title);
                $('#selectedPackagePerson').val(person);

                // Update submit button text
                $('.packageClick').html(`Book Now &nbsp; (₹${price}/-)`);
            }
