...
@section('title', translate('Counselling_order_list'))
@section('content')
      ...
      <div class="card __card d-none d-lg-flex web-direction customer-profile-orders">
        <div class="card-body">
          <div class="d-flex align-items-center justify-content-between gap-2 mb-0 mb-md-3">
            <h5 class="font-bold mb-0 fs-16">{{ translate('Counselling_order_list') }}</h5>
          </div>
                ...
                <tr>
                  <td class="bodytr">
                    <div class="media-order">
                      <a href="{{ route('account-counselling-order-details', ['order_id' => $counsellingorder->order_id]) }}" class="d-block position-relative">
                      <img alt="{{ translate('shop') }}" src="{{ getValidImage(path: 'storage/app/public/pooja/thumbnail/' . $counsellingorder->thumbnail, type: 'pooja') }}">
                      </a>
                      <div class="cont text-start">
                        <h6 class="font-weight-bold m-0 mb-1">
                          <a href="{{ route('account-counselling-order-details', ['order_id' => $counsellingorder->order_id]) }}" class="fs-14 font-semibold">
                          {{ translate('order') }} #{{ $counsellingorder['order_id'] }}
                          </a>
                        </h6>
                  ...
                  <td class="bodytr">
                    @if ($counsellingorder->is_edited == 1)
                  ...
                  <td class="bodytr">
                    <span class="status-badge rounded-pill __badge badge-soft-{{ $counsellingorder->order_status == 0
                      ? 'primary'
                      : ($counsellingorder->order_status == 1
                      ? 'success'
                      : ($counsellingorder->order_status == 2
                      ? 'warning'
                      : ($counsellingorder->order_status == 3
                      ? 'danger'
                      : ($counsellingorder->order_status == 6
                      ? 'warning'
                      : ($counsellingorder->order_status == 4
                      ? 'info'
                      : ($counsellingorder->order_status == 5
                      ? 'secondary'
                      : 'dark')))))) }} fs-12 font-semibold text-capitalize">
                    {{ $counsellingorder->order_status == 0
                    ? 'Pending'
                    : ($counsellingorder->order_status == 1
                    ? 'Completed'
                    : ($counsellingorder->order_status == 2
                  ...
                  <td class="bodytr">
                    <div class="__btn-grp-sm flex-nowrap">
                      @if ($counsellingorder->is_edited == 0)
                      <a href="{{ route('account-counselling-order-user-detail', ['order_id' => $counsellingorder->order_id]) }}"
                      ...
                      <a href="{{ route('consultation-generate-invoice-service', ['id' =>$counsellingorder->id]) }}" title="{{ translate('download_invoice') }}"
              ...
              <div class="media-order">
                ...
                <div class="cont text-start">
                  ...
                  <span class="status-badge rounded-pill __badge badge-soft-{{ $counsellingorder->order_status == 0
                    ...
                    ? 'warning'
                    : 'dark')) }} fs-12 font-semibold text-capitalize">
                  {{ $counsellingorder->status == 0
                  ? 'Pending'
                  : ($counsellingorder->status == 1
                  ? 'Completed'
                  : ($counsellingorder->order_status == 2
                  ? 'Canceled'
                  : 'Unknown')) }}
