                    ...
                    <div class="card-body">
                        ...
                        @if ($order)
                            <form class="needs-validation" id="sankalp_check"
                                action="{{ route('sanklpUpdate', $order['order_id']) }}" method="post">
                                @csrf
                                ...
                                <div class="row">
                                    @php
                                        ...
                                        $personCount = $order['packages']['person'];
                                    @endphp

                                    @for ($person = 1; $person <= $personCount; $person++)
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                <label class="form-label font-semibold">Family Member</label>
                                                <input class="form-control text-align-direction" type="text"
                                                    name="members[]" value="{{ $members[$person - 1] ?? '' }}"
                                                    pattern="^[A-Za-z\s]+$" placeholder="Family Member {{ $person }}"
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" type="text" name="gotra" id="GotraId"
                                                ...
                                                {{ $order['is_edited'] == '1' ? 'readonly' : '' }} pattern="^[A-Za-z\s]+$"
                                                required>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="row">
                                    @php
                                        $isPrashad = $order->is_prashad ?? 0; // Fetch value from the database
                                    @endphp
                                    <div class="col-md-9">
                                        <h2 class="mt-4 mb-3  text-lg-left mobile-fs-20 fs-18 font-bold">
                                ...
                                <div class="row hideable-div mt-3">
                                    ...
                                    <input type="hidden" name="booking_date" value="{{ $bookingdate }}"id="booking-date">
                                    <input type="hidden" name="type" value="{{ $type }}" id="type">
                                    <input type="hidden" name="payment_type" value="P" id="payment-type">
                                    <input type="hidden" name="warehouse_id" value="61202" id="warehouse-id">
            ...
            if ($('#is_prashad').val() == '1') {
                ...
                requiredFields.forEach(function(field) {
                    ...
                    if ((field === '#state' || field === '#city') && !/^[A-Za-z\s]+$/.test(
                            value)) {
                        $(field).css('border', '1px solid red');
                        toastr.error("State and City should contain only letters.");
                        return false;
                        isValid = false;
                    }
                });
