                                                    ...
                                                    @foreach ($choice->options as $index => $option)
                                                        <div>
                                                            <div class="for-mobile-capacity">
                                                                <input type="radio" id="{{ $choice->name }}-{{ str_replace(' ','_',$option) }}" name="{{ $choice->name }}" value="{{ $option }}" @if($index == 0) checked @endif >
                                                                <label class="__text-12px" for="{{ $choice->name }}-{{ str_replace(' ','_',$option) }}">{{ $option }}</label>
                                                            </div>
                                                        </div>
                                                    @endforeach
                                                </div>
                                    ...
                                    <div class="__btn-grp mt-2 mb-3 d-none d-sm-flex">
                                        @if(($product->added_by == 'seller' && ($sellerTemporaryClose || (isset($product->seller->shop) && $product->seller->shop->vacation_status && $currentDate >= $sellerVacationStartDate && $currentDate <= $sellerVacationEndDate))) ||
                                         ($product->added_by == 'admin' && ($inHouseTemporaryClose || ($inHouseVacationStatus && $currentDate >= $inHouseVacationStartDate && $currentDate <= $inHouseVacationEndDate))))
                                            <button class="btn" type="button" disabled style="background-color:#00bd68; color:#fff !important;">
                                                {{translate('buy_now')}}
                                            </button>
                                        ...
                                        @else
                                            <button
                                                class="btn element-center btn-gap-{{Session::get('direction') === "rtl" ? 'left' : 'right'}} action-buy-now-this-product"
                                                type="button" style="background-color:#00bd68; color:#fff !important;">
            ...
            <div class="d-flex flex-column gap-1 py-2">
                ...
                <div class="d-flex gap-3 justify-content-center">
                    @if(($product->added_by == 'seller' && ($sellerTemporaryClose || (isset($product->seller->shop) && $product->seller->shop->vacation_status && $currentDate >= $sellerVacationStartDate && $currentDate <= $sellerVacationEndDate))) ||
                        ($product->added_by == 'admin' && ($inHouseTemporaryClose || ($inHouseVacationStatus && $currentDate >= $inHouseVacationStartDate && $currentDate <= $inHouseVacationEndDate))))
                        <button
                            class="btn btn-sm btn-gap-{{Session::get('direction') === "rtl" ? 'left' : 'right'}}"
                            type="button" disabled style="background-color:#00bd68; color:#fff !important;">
                            {{translate('buy_now')}}
                        </button>
                        <button
                    ...
                    @else
                        <button
                            class="btn btn-sm btn-gap-{{Session::get('direction') === "rtl" ? 'left' : 'right'}} action-buy-now-this-product" type="button" style="background-color:#00bd68; color:#fff !important;">
                            <span class="string-limit">{{translate('buy_now')}}</span>
                        </button>
                        <button
