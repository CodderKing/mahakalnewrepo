...
@section('content')
    ...
    <div class="container py-5 rtl text-start">
        ...
        <h3 class="mb-3 text-center font-semi-bold text-capitalize h3"> {{ translate('Partner_with_us') }}</h3>
        <form class="__shop-apply" action="{{ route('vendor.auth.registration.index') }}" id="vendor-registration-form"
            method="post" enctype="multipart/form-data">
            @csrf
            <div class="card __card mb-3 option_all_hiddens">
                <div class="card-header">
                    <div>
                        <style>
                            ...
                            .chevron-container i {
                                animation: moveChevron 1s infinite ease-in-out;
                                margin-right: 5px;
                            }

                            .chevron-container i:nth-child(1) {
                                animation-delay: 0s;
                            }
                            ...
                            .chevron-container i:nth-child(4) {
                        ...
                        <span class="h3">You Want to <span class="text-primary">Register</span> as a
                            <select class="form-control " style="display: inline;width:30%"
                                onchange="choose_options(this.value)">
                                <option value="" disabled selected>Select</option>
                                <option value="seller">Vendor</option>
                                <option value="tour">Tour</option>
                                <option value="event">Event</option>
                                <option value="trust">Trustee</option>
                            </select>
                            <a class="text-primary chevron-container"
                                style="font-size: 30px; margin-top: 4px; margin-left: 10px; position: absolute;">
                                <i class="fa fa-chevron-right"></i><i class="fa fa-chevron-right"></i><i
                                    class="fa fa-chevron-right"></i><i class="fa fa-chevron-right"></i>
                            </a>
                        </span>
                    </div>
                </div>
            ...
            <div class="shop_use_hidden d-none">
                <div class="card __card mb-3">
                    <div class="card-header">
                        <h5 class="card-title m-0 fs-16 font-semi-bold d-flex align-items-center gap-2">
                            <a
                                onclick="$('.shop_use_hidden').addClass('d-none');$('.option_all_hiddens').removeClass('d-none')"><i
                                    class="fa fa-chevron-left" aria-hidden="true"></i></a>&nbsp;&nbsp; <i
                                class="fa fa-user-o" aria-hidden="true"></i>
                            <span class="header_title_name">{{ translate('vendor_Info') }}</span>
                        </h5>
                    </div>
                    <div class="card-body">
                        <div class="row g-3">
                            ...
                            <div class="col-sm-6">
                                <input type="text" class="form-control form-control-user" id="exampleLastName"
                                    name="l_name" value="{{ old('l_name') }}" placeholder="{{ translate('last_name') }}"
                                    required>
                            </div>
                            <div class="col-sm-6">
                                <input type="email" class="form-control form-control-user" id="exampleInputEmail"
                                    name="email" value="{{ old('email') }}"
                                    placeholder="{{ translate('email_address') }}" required onkeyup="validateEmail()">
                    ...
                    <div class="card-header new_title_hidden d-none">
                        <h5 class="card-title m-0 fs-16 font-semi-bold d-flex align-items-center gap-2">
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            <div class="new_title_hidden d-none">
                                <div class="row">
                                    ...
                                    <div class="col-sm-6 mt-2">
                                        <textarea name="shop_address" class="form-control getAddress_google" id="shop_address" rows="1"
                                            placeholder="{{ translate('shop_address') }}">{{ old('shop_address') }}</textarea>
                                        <input type="hidden" name="latitude">
                                        <input type="hidden" name="longitude">
                                        <input type="hidden" name="state_name" class="state_select_geolocation">
                                        <input type="hidden" name="city_name" class="City_select_geolocation">
                                        <input type="hidden" name="country_name" class="Country_select_geolocation">
                                        <input type="hidden" name="zipcode" class="form-control zipcode">
                                    </div>
                            ...
                            <div class="col-sm-12">
                                <div class="form-group mb-0 d-flex flex-wrap justify-content-between">
                                    <label
                                        ...
                                        <span class="mb-4px d-block w-0 flex-grow pl-1">
                                            ...
                                            <a class="font-size-sm text-force-underline text-primary change_urls"
                                                target="_blank" href="{{ route('terms', ['type' => 'seller']) }}">
                                                {{ translate('terms_and_condition') }}
                                            </a>
                                        </span>
                                    </label>
                                    <div>
                                        <a href="{{ route('helpTopic', ['type' => 'seller']) }}"
                                            class="text-primary change_faqs_urls" style="text-decoration: underline;"><i
                                                class="fa fa-comments-o" aria-hidden="true"></i>&nbsp;FAQs ?</a>
                                    </div>
                                </div>
                                <input type="hidden" name="from_submit" class="select_option_type" value="seller">
                                <button type="button"
                                ...
                                <div class="text-center">
                                    <a class="small" href="{{ route('vendor.auth.login') }}">
                                        {{ translate('already_have_an_account') }} ? <span
                                            class="font-weight-bold text-primary">{{ translate('sign_in') }}</span>
                                    </a>
                                </div>
...
@push('script')
    ...
    <script>
        function choose_options(name) {
            if (name == 'seller') {
                $(".shop_use_hidden").removeClass('d-none');
                $(".new_title_hidden").removeClass('d-none');
                $(".header_title_name").text("{{ translate('vendor_Info') }}");
                $("#get-confirm-and-cancel-button-text").data('message',
                    "{{ translate('want_to_apply_as_a_vendor') . '?' }}");
                $(".change_urls").attr('href', "{{ route('terms', ['type' => 'seller']) }}");
                $(".change_faqs_urls").attr('href', "{{ route('helpTopic', ['type' => 'seller']) }}");
            } else if (name == 'event') {
                $('.shop_use_hidden').removeClass('d-none');
                $(".new_title_hidden").addClass('d-none');
                $(".header_title_name").text("{{ translate('Event_Organizer_Info') }}");
                $("#get-confirm-and-cancel-button-text").data('message',
                    "{{ translate('want_to_apply_as_a_Event_Organizer') . '?' }}");
                $(".change_urls").attr('href', "{{ route('terms', ['type' => 'event']) }}");
                $(".change_faqs_urls").attr('href', "{{ route('helpTopic', ['type' => 'event']) }}");
            } else if (name == 'tour') {
                $('.shop_use_hidden').removeClass('d-none');
                $(".new_title_hidden").addClass('d-none');
                $(".header_title_name").text("{{ translate('Traveller_Info') }}");
                $("#get-confirm-and-cancel-button-text").data('message',
                    "{{ translate('want_to_apply_as_a_Traveller') . '?' }}");
                $(".change_urls").attr('href', "{{ route('terms', ['type' => 'tour']) }}");
                $(".change_faqs_urls").attr('href', "{{ route('helpTopic', ['type' => 'tour']) }}");
            } else if (name == 'trust') {
                $(".new_title_hidden").addClass('d-none');
                $('.shop_use_hidden').removeClass('d-none');
                $(".header_title_name").text("{{ translate('Trustees_Info') }}");
                $("#get-confirm-and-cancel-button-text").data('message',
                    "{{ translate('want_to_apply_as_a_Trustees') . '?' }}");
                $(".change_urls").attr('href', "{{ route('terms', ['type' => 'trustee']) }}");
                $(".change_faqs_urls").attr('href', "{{ route('helpTopic', ['type' => 'trustee']) }}");
            }
            $(".select_option_type").val(name);
            $('.option_all_hiddens').addClass('d-none');
        }
    </script>
    ...
    <script>
        $(document).ready(function() {
            const inputElement = $('textarea[name="shop_address"].form-control.getAddress_google')[0];
            ...
            $(".getAddress_google").on('input', function() {
                ...
                autocomplete.addListener('place_changed', () => {
                    ...
                    $('input[type="hidden"][name="zipcode"].form-control.zipcode').val(zipcode);
