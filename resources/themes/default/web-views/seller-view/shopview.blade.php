...
@push('css_or_js')
    @if($shop['id'] != 0)
        ...
        <meta property="og:url" content="{{route('shopView',[$shop['slug']])}}">
    @else
    ...
    @if($shop['id'] != 0)
        ...
        <meta property="twitter:title" content="{{route('shopView',[$shop['slug']])}}"/>
        <meta property="twitter:url" content="{{route('shopView',[$shop['slug']])}}">
    @else
...
@section('content')
    ...
    <span id="shop-sort-by-filter-url" data-url="{{url('/')}}/shopView/{{$shop['slug']}}"></span>
