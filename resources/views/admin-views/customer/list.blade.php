                ...
                <table style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};"
                    ...
                    <thead class="thead-light thead-50 text-capitalize">
                        <tr>
                            ...
                            <th>{{ translate('joined_At') }}</th>
                            <th>{{ translate('Address') }}</th>
                            <th>{{ translate('total_Order') }} </th>
                            <th>{{ translate('app_Status') }} </th>
                            @if (Helpers::modules_permission_check('Customers', 'Customer List', 'status'))
                                <th class="text-center">{{ translate('block') }} / {{ translate('unblock') }}</th>
                            @endif
                            @if (Helpers::modules_permission_check('Customers', 'Customer List', 'detail') ||
                                    Helpers::modules_permission_check('Customers', 'Customer List', 'delete'))
                            ...
                            <tr>
                                <td>
                                    <div class="d-flex">
                                        ...
                                        @if (!App\Models\AppDownload::where('mobile_no', $customer->phone)->exists())
                                            <input type="checkbox" class="app-link ml-2" name="app_link"
                                                value="{{ $customer->id }}" id=""
                                                onclick="limitCheckboxSelection()">
                                        @endif
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                    {{App\Models\AppDownload::where('mobile_no',$customer->phone)->exists()?App\Models\AppDownload::where('mobile_no',$customer->phone)->get()->value('city'):'NA'}}
                                </td>
                                <td>
                                    @php
                                        ...
                                        $toCount = App\Models\TourOrder::where('user_id',$customer->id)->count();
                                        $evoCount = App\Models\EventOrder::where('user_id',$customer->id)->count();
                                        $doCount = App\Models\DonateAllTransaction::where('user_id',$customer->id)->count();
                                        $koCount = App\Models\BirthJournalKundali::where('user_id',$customer->id)->count();
                                    @endphp
                                    <label class="btn text-info bg-soft-info font-weight-bold px-3 py-1 mb-0 fz-12">
                                        {{ $customer->orders_count + $poCount + $coCount + $opoCount + $toCount + $evoCount + $doCount + $koCount }}
                                    </label>
                                </td>
                                <td>
                                    <span
                                        class="badge badge-soft-{{ App\Models\AppDownload::where('mobile_no', $customer->phone)->exists() ? 'success' : 'danger' }} fz-12">
                                        {{ App\Models\AppDownload::where('mobile_no', $customer->phone)->exists() ? 'Yes' : 'No' }}
                                    </span>
                                </td>
                                @if (Helpers::modules_permission_check('Customers ', 'Customer List', 'status'))
                                    <td>
                                        ...
                                        @else
                                            <form action="{{ route('admin.customer.status-update') }}" method="post"
                                                id="customer-status{{ $customer['id'] }}-form"
                                ...
                                @if (Helpers::modules_permission_check('Customers', 'Customer List', 'detail') ||
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center gap-2">
                                            ...
                                            @if (Helpers::modules_permission_check('Customers', 'Customer List', 'delete'))
                                                @if ($customer['id'] != '0')
...
@push('script')
    <script>
        function limitCheckboxSelection() {
            var checkedBoxes = $(".app-link:checked");

            if (checkedBoxes.length > 10) {
                checkedBoxes.last().prop("checked", false);
                alert("You can select a maximum of 10 checkboxes.");
            }
        }
        ...
        function sendAppLink() {
            ...
            $(".app-link:checked").each(function() {
            ...
            if (selectedCustomer.length > 10) {
                alert("Do not select more than 10 customer.");
                return;
            }

            $.ajax({
                type: "post",
                url: "{{ route('admin.customer.sendapplink') }}",
                data: {
