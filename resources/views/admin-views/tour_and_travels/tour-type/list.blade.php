...
@section('title', translate('tour_type_setting'))
...
<div class="content container-fluid">
   ...
   <div class="row">
      ...
      <div class="col-md-12 mb-3">
         <div class="card">
            <div class="card-body">
               <form action="{{ route('admin.tour_type.store') }}" method="post" enctype="multipart/form-data">
                  @csrf
                  <!-- Language tabs -->
                        ...
                        <div class="form-group {{$lang != $defaultLanguage ? 'd-none':''}} form-system-language-form" id="{{$lang}}-form">
                           <div class="row">
                              <div class="col-md-12">
                                 ...
                                 <input type="text" name="name[]" value="{{ old('name.'.$loop->index)}}" class="form-control" placeholder="{{ translate('enter_tour_Type_name') }}" {{$lang == $defaultLanguage? 'required':''}}>
               ...
               <div class="row align-items-center">
                  <div class="col-sm-4 col-md-6 col-lg-8 mb-2 mb-sm-0">
                     <h5 class="mb-0 d-flex align-items-center gap-2">{{ translate('Package_list') }}
                        <span class="badge badge-soft-dark radius-50 fz-12">{{ ($getDatalist->total()??'') }}</span>
                     </h5>
                  </div>
            ...
            <div class="text-start">
               <div class="table-responsive">
                  <table id="datatable"
                     ...
                     <tbody>
                        <!-- Loop through items -->
                        @foreach($getDatalist as $key => $items)
                        <tr>
                           <td>{{$getDatalist->firstItem()+$key}}</td>
                           <td>{{ $items['name'] }}</td>
                           <td>
                              <form action="{{route('admin.tour_type.status-update') }}" method="post" id="items-status{{$items['id']}}-form">
                                 @csrf
                                 ...
                                 <label class="switcher mx-auto">
                                    <input type="checkbox" class="switcher_input toggle-switch-message" name="status"
                                       ...
                                       data-on-message="<p>{{ translate('if_enabled_this_tour_type_service_will_be_available_on_the_website_and_customer_app') }}</p>"
                                       data-off-message="<p>{{ translate('if_disabled_this_tour_type_service_will_be_hidden_from_the_website_and_customer_app') }}</p>">
                           ...
                           <td>
                              <div class="d-flex justify-content-center gap-2">
                                 <a class="btn btn-outline-info btn-sm square-btn" title="{{ translate('edit') }}" href="{{route('admin.tour_type.update',[$items['id']])}}">
                                    <i class="tio-edit"></i>
                                 </a>
...
<span id="route-admin-tour_package-delete" data-url="{{ route('admin.tour_type.delete') }}"></span>
...
<div class="position-fixed bottom-0 end-0 p-3" style="z-index: 5">
   <div id="panchangmoonimage-deleted-message" class="toast hide" role="alert" aria-live="assertive"
      ...
      <div class="toast-body">
         {{ translate('Tour_type_service_deleted') }}
      </div>
   </div>
</div>
@endsection
               ...
               success: function(response) {
                    if(response.status == 1){
                     ...
                     positionClass: 'toast-bottom-left'
                        ...
                        toastr.error("{{ translate('name_is_not_deleted_because_the_visit_is_active') }}", '', {
                     positionClass: 'toast-bottom-left'
                  });
                    }
