                            ...
                            <select name="venue_id" class="form-control" onchange="this.form.submit()">
                                ...
                                @if (!empty($venue_data) && json_decode($venue_data))
                                    @foreach (json_decode($venue_data) as $vl)
                                        <option value="{{ $vl->id }}"
                                            {{ $vl->id == request('venue_id') ? 'selected' : '' }}>
                                            {{ $vl->en_event_venue }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                    ...
                    <table id="datatable"
                                    ...
                                    <tr>
                                                ...
                                                @if (!empty($getData['package_list']) && json_decode($getData['package_list'], true))
                                                    @foreach (json_decode($getData['package_list'], true) as $val)
                                                        @if ($val['id'] == $items['package_id'])
                                                            ...
                                                            <a role='tooltip' data-toggle="tooltip" data-html="true"
                                                                title="
                                        @if (!empty($val['package_id'] ?? '')) @foreach ($val['package_id'] as $pn)
                                        ...
                                        <td>
                                            @if ($items['company'])
                                                ...
                                                <span> {{ Str::limit($items['company']['company_name'] ?? '', 20) }}
                                                </span><br>
                                                <span> {{ $items['company']['phone_no'] ?? '' }}</span><br>
                                            @else
                                                <span class="btn btn-outline-danger btn-sm mb-1">Cab Not
                                                    assigned</span><br>
                                            @endif
                                            <span> {{ date('d M,Y', strtotime($items['pickup_date'] ?? '')) }}
                                                {{ $items['pickup_time'] ?? '' }}</span>
                                        </td>
                        ...
                        <tfoot>
                            ...
                            <th>
                                @php
                                    if (!empty(request('cab_assign'))) {
                                        $sub_amount = \App\Models\TourOrder::where('amount_status', 1)
                                            ->where('tour_id', $getData['id'])
                                            ->where('status', '!=', 2)
                                            ->where('cab_assign', request('cab_assign'))
                                            ->sum('amount');
                                        $coupon_amount = \App\Models\TourOrder::where('amount_status', 1)
                                            ->where('tour_id', $getData['id'])
                                            ->where('status', '!=', 2)
                                            ->where('cab_assign', request('cab_assign'))
                                            ->sum('coupon_amount');
