...
<div class="content container-fluid">
    ...
    <form class="product-form text-start" action="{{ route('admin.tour_visits.edit') }}" method="POST" enctype="multipart/form-data" id="services_form">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class='col-md-12 form-group'>
                        <label class="title-color font-weight-bold h3" for="Language">{{ translate('Package') }}&nbsp;&nbsp;&nbsp; <input type="checkbox" name="is_person_use" value="{{ old('is_person_use',$getData['is_person_use']) }}" {{ ((old('is_person_use',$getData['is_person_use']) == 1)?'checked':'' ) }} onclick="if(this.checked) {  
                    ...
                    <div class="col-md-12 form-group person_package_includes {{ ((old('is_person_use',$getData['is_person_use']) == 1)?'':'d-none' ) }}">
                        <div class="d-flex">
                            ...
                            <input type="checkbox" name="include_package[food]" id="" {{ old('include_package.food',$includePackages['food']??0 == 1) ? 'checked' : '' }}>&nbsp;Food &nbsp;&nbsp;&nbsp;
                            <input type="checkbox" name="include_package[hotel]" id="" {{ old('include_package.hotel',$includePackages['hotel']??0 == 1) ? 'checked' : '' }}>&nbsp;Accomadation &nbsp;&nbsp;&nbsp;
                        </div>
                    </div>
                    <div class="col-md-6 form-group add_persons_append_multi persons_divShow  {{ ((old('is_person_use',$getData['is_person_use']) == 1)?'':'d-none' ) }}">
                        ...
                        @for ($i = 0; $i < $totalRows_cab; $i++)
                            ...
                            <div class="col-4">
                                <input type='text' class="form-control" name="person_price[]" value="{{ old('person_price.' . $i, $oldData[$i]['price'] ?? '') }}" onkeyup="this.value = this.value.replace(/[^0-9]/g, '' )" placeholder="{{ translate('enter_Price') }}">
                            </div>
                            <div class="col-1 p-0">
                                @if($i == 0)
            ...
            <div class="col-md-6 form-group add_persons_transport_append_multi persons_transport_divShow  {{ ((old('is_person_use',$getData['is_person_use']) == 1)?'':'d-none' ) }}">
                <div class="row">
                    <div class="col-12">
...
<script type="text/javascript">
    ...
    function add_new_cab_html() {
        ...
        <div class="row mt-2">
            ...
            <div class="col-1 p-0">
                <a class='btn btn-danger btn-sm p-1 mt-2' onclick="remove_html(this)"><i class='tio-remove'></i></a>
                 <a class='btn btn--primary btn-sm p-1 mt-2 cab-ex-distance-charge${totalRows} {{((old("use_date",$getData['use_date']) == 3)?"":"d-none" )}} specialTourwithoutdate' onclick="cab_ex_distance_model('${totalRows}')"><i class="tio-bonnet_open"> bonnet_open </i></a>
