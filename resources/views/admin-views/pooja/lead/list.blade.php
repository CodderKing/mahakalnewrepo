...
@section('title', translate('pooja_Lead_List'))
@push('css_or_js')
                                ...
                                <tbody>
                                    @foreach ($leads as $key => $lead)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $lead['leadno'] }}</td>
                                            <td>{{ isset($lead['services']['name']) ? Str::limit($lead['services']['name'], 20) : '' }}
                                            </td>
                                            ...
                                            <td>
                                                <div class="d-flex justify-content-center gap-2">
                                                    ...
                                                    <a href="{{ route('admin.pooja.orders.lead-delete', ['id' => $lead['id']]) }}"
                                                    ...
                                                    <a href="javascript:void(0)"
                                                        ...
                                                        data-type="{{ $lead['type'] }}" data-leadsId="{{ $lead['id'] }}"
                                                        onclick="followUp(this)" data-toggle="tooltip"
                                                        aria-label="Follow Up" data-bs-original-title="Follow Up"
                                                        title="Follow Up">
                                                ...
                                                <div class="d-flex justify-content-center gap-2 pt-2">
                                                    <a href="javascript:0"
                                                        ...
                                                        data-leadsId="{{ $lead['id'] }}" data-type="{{ $lead['type'] }}"
                                                ...
                                                <div class="d-flex justify-content-center gap-2 pt-2">
                                                    <a href="{{ route('admin.pooja.orders.send-whatsapp-leads', ['id' => $lead['id']]) }}"
                                                        class="btn btn-icon bg-label-success waves-effect waves-light myactionbtn"
                                                        data-toggle="tooltip" aria-label="whatsapp"
                                                        data-bs-original-title="whatsapp"><i class="tio-whatsapp"
                                                            title="whatsapp"></i>
                                                        <span
                                                            class="btn-status btn-sm-status btn-status-danger">{{ $lead['whatsapp_hit'] }}</span>
                                                    </a>
                                                    <a href="{{ route('admin.whatsapp.pooja-template') }}"
    ...
    <div class="modal fade" id="followUpModal" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.pooja.orders.lead-follow-up') }}" method="POST">
                    @csrf
                    @php
                        if (auth('admin')->check()) {
...
@push('script')
    ...
    <script>
        function followHistory(that) {
            ...
            var row = "";
            $.ajax({
                url: "{{ url('admin/pooja/orders/get-follows-list') }}/" + leadId,
                method: 'GET',
                //   data:{id:leadId},
