    ...
    <div class="content container-fluid">
        ...
        {{-- <div class="card mb-3 remove-card-shadow">
            <div class="card-body col-md-4">     
                <form id="pooja-type-form" action="{{url()->current()}}" method="GET">
                    <select name="pooja_type" id="" class="form-control" onchange="$('#pooja-type-form').submit();">
                        <option value="all" {{ (request()->query('pooja_type') === 'all' || !request()->has('pooja_type')) ? 'selected' : '' }}>All</option>
                        <option value="pooja" {{ request()->query('pooja_type') === 'pooja' ? 'selected' : '' }}>Normal Pooja</option>
                        <option value="instant-pooja" {{request()->query('pooja_type') === 'instant-pooja'?'selected':''}}>Instant Pooja</option>
                    </select>
                </form>
            </div>
        </div> --}}
