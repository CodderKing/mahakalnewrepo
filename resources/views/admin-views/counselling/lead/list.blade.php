...
@section('title', translate('counselling_Lead_List'))
@push('css_or_js')
    ...
    <div class="content container-fluid">
        <div class="mb-3">
            <h2 class="h1 mb-0 d-flex gap-2">
                ...
                {{ translate('counselling_Lead_List') }}
                            ...
                            <table
                                ...
                                <thead class="thead-light thead-50 text-capitalize">
                                    <tr>
                                        ...
                                        <th>{{ translate('Counselling') }}</th>
                                        <th>{{ translate('Customer') }}</th>
                                        <th>{{ translate('Lead_Date') }}</th>
                                        <th>{{ translate('Follow_By') }}</th>
                                        <th>{{ translate('Follow_Date') }}</th>
                                        <th>{{ translate('Next_Date') }}</th>
                                        @if (Helpers::modules_permission_check('Consultation Order', 'Lead', 'delete') ||
                                                Helpers::modules_permission_check('Consultation Order', 'Lead', 'follow-up') ||
                                                Helpers::modules_permission_check('Consultation Order', 'Lead', 'follow-up-history') ||
                                                Helpers::modules_permission_check('Consultation Order', 'Lead', 'call'))
                                ...
                                <tbody>
                                    @foreach ($leads as $key => $lead)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $lead['leadno'] }}</td>
                                            <td>{{ Str::limit($lead['services']['name'], 20) }}</td>
                                            ...
                                            @if (Helpers::modules_permission_check('Consultation Order', 'Lead', 'delete') ||
                                                ...
                                                <td>
                                                    <div class="d-flex justify-content-center gap-2">
                                                        ...
                                                        @if (Helpers::modules_permission_check('Consultation Order', 'Lead', 'delete'))
                                                            <a href="{{ route('admin.counselling.order.lead-delete', ['id' => $lead['id']]) }}"
                                                        ...
                                                        @if (Helpers::modules_permission_check('Consultation Order', 'Lead', 'follow-up'))
                                                    ...
                                                    <div class="d-flex justify-content-center gap-2 pt-2">
                                                        @if (Helpers::modules_permission_check('Consultation Order', 'Lead', 'follow-up-history'))
                                                            <a href="javascript:0"
                                                        ...
                                                        @endif
                                                        @if (Helpers::modules_permission_check('Consultation Order', 'Lead', 'call'))
                                                    ...
                                                    <div class="d-flex justify-content-center gap-2 pt-2">
                                                        <a href="{{ route('admin.counselling.order.orders.send-whatsapp-leads', ['id' => $lead['id']]) }}"
                                                            class="btn btn-icon bg-label-success waves-effect waves-light myactionbtn"
                                                        ...
                                                        <a href="{{ route('admin.whatsapp.counsltancy-template') }}"
    ...
    <div class="modal fade" id="followUpModal" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.counselling.order.lead-follow-up') }}" method="POST">
                    @csrf
                    @php
                        if (auth('admin')->check()) {
...
@push('script')
    ...
    <script>
        function followHistory(that) {
            ...
            var row = "";
            $.ajax({
                url: "{{ url('admin/counselling/order/get-follows-list') }}/" + leadId,
                method: 'GET',
                //   data:{id:leadId},
