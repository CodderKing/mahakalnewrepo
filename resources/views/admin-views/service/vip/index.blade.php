...
@section('title', translate('VIP_Pooja|List'))
@push('css_or_js')
...
@section('content')
                ...
                <div class="mb-3">
                    <h2 class="h1 mb-0 d-flex gap-2">
                        <img width="20" src="{{ dynamicAsset(path: 'public/assets/back-end/img/pooja/poojas.png') }}"
                            alt="">{{ translate('VIP_Pooja|List') }}<span
                            class="badge badge-soft-dark radius-50 fz-14"></span>
                    </h2>
                </div>
            </div>
            <div class="col-lg-8 mt-3 mt-lg-0 d-flex flex-wrap gap-3 justify-content-lg-end">
                @if (Helpers::modules_permission_check('Pooja Managment', 'VIP Pooja', 'add'))
                    <a href="{{ route('admin.service.vip.add-new') }}" class="btn btn--primary">
                        <i class="tio-add"></i><span class="text">{{ translate('add_VIP_puja') }}</span>
                    </a>
                @endif
            </div>
        </div>
            ...
            <div class="row g-2">
                    ...
                    <div class="card card-body h-100 justify-content-center">
                        <div class="d-flex gap-2 justify-content-between align-items-center">
                            <div class="d-flex flex-column align-items-start">
                                <h3 class="mb-1 fz-24"> {{ \App\Models\Vippooja::count() }}</h3>
                ...
                <div class="col-md-3">
                    <div class="card card-body h-100 justify-content-center">
                        <div class="d-flex gap-2 justify-content-between align-items-center">
                            <div class="d-flex flex-column align-items-start">
                                <h3 class="mb-1 fz-24"> {{ \App\Models\Vippooja::where('status', 1)->count() }}</h3>
                ...
                <div class="col-md-3">
                    <div class="card card-body h-100 justify-content-center">
                        <div class="d-flex gap-2 justify-content-between align-items-center">
                            <div class="d-flex flex-column align-items-start">
                                <h3 class="mb-1 fz-24">
                                    {{ \App\Models\Vippooja::where('is_anushthan', 0)->where('status', 1)->count() }}</h3>
                                <div class="text-capitalize mb-0">Total VIP Pooja</div>
                            </div>
                            <div>
                                <img width="40"
                ...
                <div class="col-md-3">
                    <div class="card card-body h-100 justify-content-center">
                        <div class="d-flex gap-2 justify-content-between align-items-center">
                            <div class="d-flex flex-column align-items-start">
                                <h3 class="mb-1 fz-24">
                                    {{ \App\Models\Vippooja::where('is_anushthan', 1)->where('status', 1)->count() }}</h3>
                                <div class="text-capitalize mb-0">Total Anustthan</div>
                            </div>
                            <div>
                                <img width="40"
                        ...
                        <div class="table-responsive">
                            <table id="myTable"
                                ...
                                <thead class="thead-light thead-50 text-capitalize">
                                    <tr>
                                        <th>{{ translate('SL') }}</th>
                                        <th>{{ translate('vip_ritual_name') }}</th>
                                        <th>{{ translate('type_of_pooja') }}</th>
                                        <th>{{ translate('prashad_assign') }}</th>
                                        @if (Helpers::modules_permission_check('Pooja Managment', 'VIP Pooja', 'status'))
                                            <th class="text-center">{{ translate('status') }}</th>
                                        @endif
                                        @if (Helpers::modules_permission_check('Pooja Managment', 'VIP Pooja', 'edit') ||
                                                Helpers::modules_permission_check('Pooja Managment', 'VIP Pooja', 'delete'))
                                            <th class="text-center"> {{ translate('action') }}</th>
                                        @endif
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($vippooja as $key => $item)
                                        <tr>
                                            <td>{{ $vippooja->firstItem() + $key }}</td>
                                            <td>
                                                {{-- {{ route('admin.service.view',['addedBy'=>$item['added_by'],'id'=>$item['id']]) }} --}}
                                                <a href="{{ route('admin.service.vip.view', ['addedBy' => $item['added_by'], 'id' => $item['id']]) }}"
                                                    data-addedby="{{ $item['added_by'] }}" data-id="{{ $item['id'] }}"
                                                    class="media align-items-center gap-2 view-service">
                                                    <img src="{{ getValidImage(path: 'storage/app/public/pooja/vip/thumbnail/' . $item['thumbnail'], type: 'backend-product') }}"
                                            ...
                                            <td>
                                                @if ($item['is_anushthan'] == 0)
                                                    {{ translate('vip_pooja') }}
                                                @elseif ($item['is_anushthan'] == 1)
                                                    {{ translate('anushthan') }}
                                                @endif
                                            </td>
                                            <td>
                                            ...
                                            @if (Helpers::modules_permission_check('Pooja Managment', 'VIP Pooja', 'status'))
                                                <td>
                                                    <form action="{{ route('admin.service.vip.status-update') }}"
                                                        method="post" id="vip-status{{ $item['id'] }}-form">
                                                        @csrf
                                                        <input type="hidden" name="id" value="{{ $item['id'] }}">
                                                        <label class="switcher mx-auto">
                                                            <input type="checkbox"
                                                                class="switcher_input toggle-switch-message" name="status"
                                                                id="vip-status{{ $item['id'] }}" value="1"
                                                                {{ $item['status'] == 1 ? 'checked' : '' }}
                                                                data-modal-id = "toggle-status-modal"
                                                                data-toggle-id = "vip-status{{ $item['id'] }}"
                                                                data-on-image = "vip-status-on.png"
                                                                data-off-image = "vip-status-off.png"
                                                ...
                                                <td>
                                                    <div class="d-flex justify-content-center gap-2">
                                                        <a class="btn btn-outline-warning btn-sm square-btn"
                                                            title="{{ translate('Prashadam') }}" href="javascript:(0);"
                                                            data-id="{{ $item->id }}"
                                                            onclick="prashadam_model(this)">
                                                        ...
                                                        @if (Helpers::modules_permission_check('Pooja Managment', 'VIP Pooja', 'edit'))
                                                            <a class="btn btn-outline-info btn-sm square-btn"
                                                                ...
                                                                href="{{ route('admin.service.vip.update', [$item['id']]) }}">
                                                                <i class="tio-edit"></i>
                                                            </a>
                                                        @endif

                                                        @if (Helpers::modules_permission_check('Pooja Managment', 'VIP Pooja', 'delete'))
                                                            <a class="btn btn-outline-danger btn-sm delete delete-data"
                                                                href="javascript:" data-id="vip-{{ $item['id'] }}"
                                                        ...
                                                        <form
                                                            action="{{ route('admin.service.vip.delete', [$item['id']]) }}"
                                                            method="post" id="vip-{{ $item['id'] }}">
                                                            @csrf @method('delete')
                                                        </form>
                                                        ...
                                                        <a class="btn btn-outline-primary btn-sm open-whatsapp-modal"
                                                            ...
                                                            data-id="{{ $item['id'] }}"
                                                            data-slug="{{ $item['slug'] }}"
                                                            data-is_anushthan="{{ $item['is_anushthan'] }}"
    ...
    <span id="route-admin-rashi-status-update" data-url="{{ route('admin.service.vip.status-update') }}"></span>
    <!-- Modal Structure -->
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="vipModalLabel">Service Details</h5>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="">
                        ...
                        <select class="js-select2-custom form-control add-prashadam" id="add-prashadam"
                            name="prashadam_id" required>
                            <option value="" disabled>{{ translate('Select_Prasad') }}</option>
                            @if (count($prashadamList) > 0)
                                @foreach ($prashadamList as $PraItem)
                                <option value="{{ $PraItem['id'] }}"  {{ $item['prashadam_id'] == $PraItem['id'] ? 'selected' : '' }}> {{ $PraItem['name'] }}</option>
                                @endforeach
                            @else
                                <option disabled>No {{ translate('Add_The_Prasadam_For_Pooja') }}</option>
                            @endif
                        </select>
                        <form action="{{ route('admin.service.vip.vip_prashad') }}" method="post"
                            id="add-prashad-form">
                            @csrf
                            <input type="hidden" name="vip_id" id="vip-id">
...
@push('script')
    ...
    <script>
        $(document).on('click', '.open-whatsapp-modal', function () {
            ...
            const is_anushthan = $(this).data('is_anushthan');

            let link = '';
            if (is_anushthan == 1) {
                link = `https://mahakal.com/anushthan/anushthan/${slug}`;
            } else {
                link = `https://mahakal.com/vip/vippooja/${slug}`;
            }
