                    ...
                    <div class="card-body">
                        <ul class="nav nav-tabs w-fit-content mb-4">
                            <li class="nav-item text-capitalize">
                                <a class="nav-link form-system-language-tab cursor-pointer {{ empty(request('show')) || request('show') == 'all' ? 'active' : '' }}"
                                    id="Allrecode-link">All Booking</a>
                            </li>
                            <li class="nav-item text-capitalize">
                                <a class="nav-link form-system-language-tab cursor-pointer {{ !empty(request('show')) && request('show') == 'event' ? 'active' : '' }}"
                                    id="trust-link">Event</a>
                            </li>
                        </ul>
                        <div class="row">
                            <div class="col-12 form-group form-system-language-form {{ empty(request('show')) || request('show') == 'all' ? '' : 'd-none' }}"
                                id="Allrecode-form">
                                <div class="px-3 py-4">
                                    <!-- Search bar -->
                                        ...
                                        <div class="col-sm-4 col-md-4 col-lg-4">
                                            <form action="{{ url()->current() }}" method="GET">
                                                <div class="input-group">
                                                    <input type="text"
                                                        class="form-control all_select_data start_date_end_date"
                                                        data-point='8' value="{{ old('start_to_end_date') }}"
                                                        name='start_to_end_date'
                                                        placeholder="{{ translate('enter_start_to_end_date') }}">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <span data-toggle="tooltip"
                                                                data-title="{{ $items['eventid']['event_name'] ?? '' }}"
                                                                role='tooltip'>{{ Str::limit($items['eventid']['event_name'] ?? '', 20) }}</span><br>
                                                            ...
                                                            <span data-toggle="tooltip"
                                                                data-title="{{ $venue_name['en_event_venue'] ?? '' }}"
                                                                role='tooltip'>{{ Str::limit($venue_name['en_event_venue'] ?? '', 20) }}</span><br>
                                                            <span>{{ $items['eventid']['organizer_by'] ?? '' }}</span>
                                                            <br>
                                                            <span>{{ $venue_name['date'] ?? '' }}</span>
                                                        </td>
                                                        <td>{{ ($items['amount'] ?? '0') + ($items['coupon_amount'] ?? '0') }}
                                                        </td>
                                                        ...
                                                        <td>
                                                            <div class="d-flex justify-content-center gap-2">
                                                                <a class="btn btn-outline-info btn-sm square-btn"
                                                                    ...
                                                                    href="{{ route('admin.event-managment.event-booking.user-booking-details', [$items['id']]) }}">
                                                                    <i class="tio-visible_outlined">visible_outlined</i>
                                                                </a>
                                                                <a title="Download Event invoice"
                                                                    class="btn btn-outline-info btn-sm square-btn"
                                                                    href="{{ route('admin.event-managment.event-booking.booking-invoice', [$items['id']]) }}">
                                            ...
                                            <fbody>
                                                <tr>
                                                    ...
                                                    <th class="font-weight-bold">
                                                        {{ ($order_array['amount'] ?? 0) + ($order_array['coupon_amount'] ?? 0) }}
                                                    </th>
                                                    <th class="font-weight-bold">{{ $order_array['coupon_amount'] ?? 0 }}
                                                    </th>
                                                    <th class="font-weight-bold">
                                                        {{ $order_array['admin_commission'] ?? 0 }}</th>
                                                    <th class="font-weight-bold">{{ $order_array['gst_amount'] ?? 0 }}</th>
                                                    <th class="font-weight-bold" colspan="2">
                                                        {{ $order_array['final_amount'] ?? 0 }}</th>
                                                </tr>
                                            </fbody>
                                        </table>
                                    </div>
                                ...
                                <div class="table-responsive mt-4">
                                    <div class="d-flex justify-content-lg-end">
                                        {!! $order_list->appends(['show' => 'all'])->links() !!}
                            ...
                            <div class="col-12 form-group form-system-language-form {{ !empty(request('show')) && request('show') == 'event' ? '' : 'd-none' }}"
                                id="trust-form">
                                <!-- trust-tab -->
                                <div class="px-3 py-4">
                                    <div class="row align-items-center">
                                        ...
                                        <table id="datatable"
                                            ...
                                            <tbody>
                                                @foreach ($getevent as $key => $items)
                                                    <tr>
                                                        ...
                                                        <td><a href="{{ route('admin.event-managment.event.event-detail-overview', [$items['event_id']]) }}"
                                                                class='font-weight-bold text-secondary'>{{ $items['eventid']['unique_id'] ?? '' }}</a>
                                                        </td>
                                                        <td>{{ $items['total_orders'] ?? '' }}</td>
                                                        <td><span role='tooltip' data-toggle="tooltip"
                                                                title="{{ $items['eventid']['event_name'] ?? '' }}">{{ Str::Limit($items['eventid']['event_name'] ?? '', 30) }}</span>
                                                        </td>
                                                        <td>{{ ($items['amount'] ?? '') + ($items['coupon_amount'] ?? '') }}
                                                        </td>
                                            ...
                                            <fbody>
                                                <tr>
                                                    <th class="font-weight-bold" colspan="4">Total Amount</th>
                                                    <th class="font-weight-bold">
                                                        {{ ($event_array['amount'] ?? 0) + ($event_array['coupon_amount'] ?? 0) }}
                                                    </th>
                                                    <th class="font-weight-bold">{{ $event_array['coupon_amount'] ?? 0 }}
                                                    </th>
                                                    <th class="font-weight-bold">
                                                        {{ $event_array['admin_commission'] ?? 0 }}</th>
                                                    <th class="font-weight-bold">{{ $event_array['gst_amount'] ?? 0 }}
                                                    </th>
                                                    <th class="font-weight-bold" colspan="2">
                                                        {{ $event_array['final_amount'] ?? 0 }}</th>
                                                </tr>
                                            </fbody>
                                        </table>
                                    </div>
