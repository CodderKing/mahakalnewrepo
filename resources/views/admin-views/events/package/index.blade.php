...
@section('title', translate('event_package'))
...
<div class="content container-fluid">
   <div class="mb-3">
      <h2 class="h1 mb-0 d-flex gap-2">
         ...
         {{ translate('event_package') }}
      </h2>
   </div>
   <div class="row">
      <!-- Form for adding new event_package -->
      <div class="col-md-12 mb-3">
         <div class="card">
            <div class="card-body">
               <form action="{{ route('admin.event-managment.event_package.store') }}" method="post" enctype="multipart/form-data">
                  @csrf
                  <!-- Language tabs -->
                  ...
                  <div class="row">
                     <div class="col-md-8">
                        ...
                        <div class="form-group {{$lang != $defaultLanguage ? 'd-none':''}} form-system-language-form"
                           ...
                           <input type="text" name="package_name[]" class="form-control" id="name" placeholder="{{ translate('enter_package_name') }}" {{$lang == $defaultLanguage? 'required':''}}>
                           <input type="hidden" name="lang[]" value="{{$lang}}" id="lang">

                           <label class="title-color" for="name">{{ translate('description') }}<span
                              class="text-danger">*</span>
                           ({{ strtoupper($lang) }})</label>
                           <textarea name="description[]" class="form-control ckeditor" placeholder="{{ translate('description') }}" {{$lang == $defaultLanguage? 'required':''}}></textarea>
                     ...
                     <div class="col-md-4 mb-4">
                        <div class="text-center">
                           <img class="upload-img-view" id="detail-viewer"
                              src="{{ getValidImage(path: 'storage/app/public/event/package/def.png', type: 'backend-product')  }}"
                              alt="">
                        </div>
                        ...
                        <tr>
                           ...
                           <td>
                              <div class="avatar-60 d-flex align-items-center rounded">
                                 <img class="img-fluid" alt="" src="{{ getValidImage(path: 'storage/app/public/event/package/' . $items['image'], type: 'backend-panchang') }}">
                              </div>
                           </td>
                           <td>
                              <!-- Form for toggling status -->
                              <form action="{{route('admin.event-managment.event_package.status-update') }}" method="post" id="items-status{{$items['id']}}-form">
                                 @csrf
                                 ...
                                 <label class="switcher mx-auto">
                                    <input type="checkbox" class="switcher_input toggle-switch-message" name="status"
                                       ...
                                       data-on-message="<p>{{ translate('if_enabled_this_event_package_will_be_available_on_the_website_and_customer_app') }}</p>"
                                       data-off-message="<p>{{ translate('if_disabled_this_event_package_will_be_hidden_from_the_website_and_customer_app') }}</p>">
                                    <span class="switcher_control"></span>
                                 </label>
                              </form>
                           </td>
                           <!-- Actions for editing and deleting event package -->
                           <td>
                              <div class="d-flex justify-content-center gap-2">
                                 <a class="btn btn-outline-info btn-sm square-btn" title="{{ translate('edit') }}" href="{{route('admin.event-managment.event_package.update',[$items['id']])}}">
                                    <i class="tio-edit"></i>
                                 </a>
                                 <a class="event_package-delete-button btn btn-outline-danger btn-sm square-btn" id="{{ $items['id'] }}">
