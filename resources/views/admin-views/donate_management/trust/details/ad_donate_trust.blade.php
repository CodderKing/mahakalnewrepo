<div class="card w-100">
    <div class="card-header">
        ...
        <a class='font-weight-bold text-secondary'>Total Amount : {{ \App\Models\DonateAllTransaction::where('type','donate_ads')->where('amount_status','1')->where('trust_id',$trust_data['id'])->sum('amount')}}</a>
    </div>
        ...
        <div class="col-md-12">
                    ...
                    <div class="col-sm-8 col-md-6 col-lg-4">
                        <form action="{{ url()->current() }}" method="GET">
                            <div class="input-group input-group-custom input-group-merge">
                                ...
                                <input type="hidden" name='type' value='donate_ad'>
            ...
            <div class="text-start">
                <div class="table-responsive">
                    <table id="datatable" class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100">
                        ...
                        <tbody>
                            <!-- Loop through items -->
                            @foreach($donate_adstrust_transaction as $key => $items)
                            <tr>
                                <td>{{$donate_adstrust_transaction->firstItem()+$key}}</td>
                        ...
                        <fbody>
                            ...
                            <th class='font-weight-bold'>₹{{ \App\Models\DonateAllTransaction::where('type','donate_ads')->where('amount_status',1)->where('trust_id',$id)->sum('amount') }}</th>
                            <th class='font-weight-bold'>₹{{ \App\Models\DonateAllTransaction::where('type','donate_ads')->where('amount_status',1)->where('trust_id',$id)->sum('admin_commission') }}</th>
                            <th class='font-weight-bold'>₹{{ \App\Models\DonateAllTransaction::where('type','donate_ads')->where('amount_status',1)->where('trust_id',$id)->sum('final_amount') }}</th>
                        </fbody>
                    </table>
                </div>
            </div>
            ...
            <div class="table-responsive mt-4">
                <div class="d-flex justify-content-lg-end">
                    {!! $donate_adstrust_transaction->links() !!}
                </div>
            </div>
            <!-- Message for no data to show -->
            @if(count($donate_adstrust_transaction) == 0)
