    ...
    <div class="row">
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed">
                ...
                <span class="order-stats__title"> {{ \App\Models\VendorSupportTicketConv::where('created_by','admin')->where('type','trust')->where('vendor_id',$relationEmployees)->whereNotNull('ticket_id')->count()}} </span>
            </a>
        </div>
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed">
                ...
                <span class="order-stats__title">{{ \App\Models\VendorSupportTicketConv::where('created_by','admin')->where('type','trust')->where('vendor_id',$relationEmployees)->where('status','open')->count()}}</span>
                </span>
            </a>
        </div>
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed">
                ...
                <span class="order-stats__title">{{ \App\Models\VendorSupportTicketConv::where('created_by','admin')->where('type','trust')->where('vendor_id',$relationEmployees)->where('status','close')->count()}} </span>
                </span>
            </a>
        </div>
    </div>
                    ...
                    <div class="card">
                        <div
                            ...
                            <div class="media gap-3">
                                ...
                                ?>
                                @if($ticket['Trust'] && $ticket['type'] == 'trust')
                                <img class="avatar avatar-lg" src="{{ getValidImage(path: 'storage/app/public/donate/trust/' . $ticket['Trust']['theme_image'] ?? '', type: 'backend-profile') }}" alt="">
                                <div class="media-body">
                                    <h6
                                        class="mb-0 {{ Session::get('direction') === 'rtl' ? 'text-right' : 'text-left' }}">
                                        {{ $ticket['Trust']['name'] ?? '' }}
                                        @if($ticket['Trust']['name'] != $ticket['Trust']['trust_name'])
                                        <br> {{ $ticket['Trust']['trust_name'] ?? '' }}
                                        @endif
                                        @if($getCountunread > 0)
                                ...
                                @if(Helpers::Employee_modules_permission('Support Management', 'From Admin', 'Status'))
                                <form action="{{ route('trustees-vendor.messages.status') }}" method="post" id="support-ticket{{ $ticket['id'] }}-form">
                                    @csrf
                            ...
                            <div
                                ...
                                <div class="text-nowrap">
                                    ...
                                    <a class="btn btn--primary"
                                        href="{{ route('trustees-vendor.message.singleTicket', $ticket['id']) }}">
