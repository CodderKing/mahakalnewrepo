...
@section('title', translate('cancel_order'))
@push('css_or_js')
                            ...
                            <table id="datatable"
                                ...
                                <thead class="thead-light thead-50 text-capitalize">
                                    <tr>
                                        ...
                                        <th class="text-center">{{translate('refund_amount')}}</th>
                                        <th class="text-center">{{translate('option')}}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @if(!empty($cancel_order) && count($cancel_order) > 0)
                                    @foreach($cancel_order as $key=>$orders)
                                    <tr>
                                                                                ...
                                                                                if (!empty($orders['booking_package']) && json_decode($orders['booking_package'] ?? '[]', true)) {
                                                                                    foreach (json_decode($orders['booking_package'], true) as $val) {
                                                                                        if ($val['type'] == 'ex_distance')
                                                                                            $ex_charges = $val['price'] ?? 0;
                                                                                        elseif ($val['type'] == 'cab') {
                                                                                            $assign_cabs = $val['id'] ?? 0;
                                                                                        }
                                                                                    }
                                                                                }
                                                                                if (!empty($orders['booking_package']) && json_decode($orders['booking_package'] ?? "[]", true)) {
                                        ...
                                        <td>
                                            <div class='row' style="width: 248px;">
                                                ...
                                                <div class="col-6">{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: (($orders['amount']??0) + ($orders['coupon_amount']??0))), currencyCode: getCurrencyCode()) }}</div>
                                                <div class="col-6">{{ translate('coupon_amount') }}</div>
                                                <div class="col-6"> {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: ($orders['coupon_amount']??0)), currencyCode: getCurrencyCode()) }}</div>
                                                <div class="col-6">{{ translate('gst_amount') }}</div>
                                                <div class="col-6">{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: ($orders['gst_amount']??0)), currencyCode: getCurrencyCode()) }}</div>
                                                <div class="col-6">{{ translate('admin_commission') }}</div>
                                                <div class="col-6">{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: ($orders['admin_commission']??0)), currencyCode: getCurrencyCode()) }}</div>
                                        ...
                                        <td>{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount:($val['refund_amount']??0)), currencyCode: getCurrencyCode()) }}</td>
                                ...
                                <tfoot>
                                    <tr>
                                        <td colspan='6'></td>
                                        <td class="text-center">{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: ( \App\Models\TourOrder::where('status',2)->where('refund_status',1)->where('traveller_id',auth('tour')->user()->relation_id)->sum('final_amount')) ), currencyCode: getCurrencyCode()) }}</td>
                                        <td>{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: ( \App\Models\TourOrder::where('status',2)->where('refund_status',1)->where('traveller_id',auth('tour')->user()->relation_id)->sum('refund_amount')) ), currencyCode: getCurrencyCode()) }}</td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                                @endif
