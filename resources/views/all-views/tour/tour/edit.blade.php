...
<div class="content container-fluid">
    ...
    <form class="product-form text-start" action="{{ route('tour-vendor.tour_visits.tour-edit') }}" method="POST" enctype="multipart/form-data" id="services_form">
        @csrf
        <div class="card">
            ...
            <div class="card-body">
                ...
                if (count($getData['translations'])) {
                    foreach ($getData['translations'] as $translations) {
                        ...
                        if ($translations->locale == $lang && $translations->key == 'number_of_day') {
                            $translate[$lang]['number_of_day'] = $translations->value;
                        }
                    }
                }
                ?>
                <div class="{{ $lang != $defaultLanguage ? 'd-none' : '' }} form-system-language-form" id="{{ $lang }}-form">
                    <div class="row mt-4">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="title-color" for="{{ $lang }}_days">{{ translate('number_of_days_of_tour') }} </label>
                                <input type="text" name="number_of_day[]" class="form-control @error('number_of_day.'.$loop->index) is-invalid @enderror " value="{{ old('number_of_day.'.$loop->index,($lang == $defaultLanguage ? $getData['number_of_day'] : $translate[$lang]['number_of_day'] ?? '')) }}" placeholder="{{ translate('number_of_day') }}" required>
                            </div>
                        </div>
                    ...
                    <div class="col-md-6 form-group add_cab_append_multi">
                        ...
                        @for ($i = 0; $i < $totalRows_cab; $i++)
                            ...
                            <div class="col-5 p-0 pr-1">
                                <input type='text' class="form-control   point_trigger46{{$i}}" name="price[{{ $i }}]" value="{{ old('price.' . $i, $oldData[$i]['price'] ?? '') }}" onkeyup="this.value = this.value.replace(/[^0-9]/g, '' );select_value(this);" data-point='point_trigger46{{$i}}' placeholder="{{ translate('enter_Price') }}">
                            </div>
                ...
                <div class="col-md-6 add_package_append_multi">
                    ...
                    @for ($i = 0; $i < $totalRows_package; $i++)
                        ...
                        <div class="col-5 p-0 pr-1">
                            <input type='text' class="form-control   point_trigger48{{$i}}" name="pprice[{{ $i }}]" value="{{ old('pprice.' . $i, $oldData[$i]['pprice'] ?? '') }}" onkeyup="this.value = this.value.replace(/[^0-9]/g, '' );select_value(this)" data-point='point_trigger48{{$i}}' placeholder="{{ translate('enter_Price') }}">
                        </div>
                ...
                <div class="row mt-2">
                    <div class='col-6 p-0 pr-1'>
                        <select class="form-control point_trigger260" name="package_id[0]" onchange="select_value(this)" data-point='point_trigger260'>
                            <option value="">Select Package</option>
                            @if($package_list)
                            @foreach($package_list as $packval)
                            <option value="{{ $packval['id'] }}" {{ (collect(old('package_id0', $oldData[0]['package_id'] ?? []))->contains($packval['id'])) ? 'selected' : '' }}>{{ $packval['name'] }} -({{ $packval['seats'] }} people)</option>
                            @endforeach
                            @endif
                        </select>
                    </div>
                    <div class="col-5 p-0 pr-1">
                        <input type='text' class="form-control   point_trigger480" name="pprice[0]" value="{{ old('pprice0', $oldData[0]['pprice'] ?? '') }}" onkeyup="this.value = this.value.replace(/[^0-9]/g, '' );select_value(this)" data-point='point_trigger480' placeholder="{{ translate('enter_Price') }}">
                    </div>
                    <div class="col-1 p-0">
                        <a class='btn btn--primary btn-sm p-1 mt-2' onclick="add_new_package_html()"><i class='tio-add'></i></a>
                    </div>
...
<script type="text/javascript">
    ...
    function add_new_cab_html() {
        ...
        <div class="row mt-2">
            ...
            <div class="col-5 p-0 pr-1">
                <input type='text' class="form-control   point_trigger4${totalRows}" name="price[${totalRows}]" value="" onkeyup="this.value = this.value.replace(/[^0-9]/g, '' );select_value(this)" data-point='point_trigger4${totalRows}' placeholder="{{ translate('enter_Price') }}">
            </div>
            <div class="col-1 p-0">
                <a class='btn btn-danger btn-sm p-1 mt-2' onclick="remove_html(this)"><i class='tio-remove'></i></a>
            </div>
    ...
    function add_new_package_html() {
        ...
        <div class="row mt-2">
            ...
            <div class="col-5 p-0 pr-1">
                <input type='text' class="form-control   point_trigger49${totalRows}" name="pprice[${totalRows}]" value="" onkeyup="this.value = this.value.replace(/[^0-9]/g, '' );select_value(this)" data-point='point_trigger49${totalRows}' placeholder="{{ translate('enter_Price') }}">
            </div>
            <div class="col-1 p-0">
                <a class='btn btn-danger btn-sm p-1 mt-2' onclick="remove_html(this)"><i class='tio-remove'></i></a>
            </div>
