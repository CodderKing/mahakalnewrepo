...
@section('content')
                                ...
                                <div class="d-flex flex-column gap-2 mt-3">
                                    <div class="order-status d-flex justify-content-sm-end gap-10 text-capitalize">
                                        ...
                                        @elseif($order['order_status'] == 'delivered' || $order['order_status'] == 'confirmed')
                                            <span
                                                class="badge badge-soft-success font-weight-bold radius-50 d-flex align-items-center py-1 px-2">{{ translate(str_replace('_', ' ', $order['order_status'])) }}</span>
                                        @else
                                            <span
                                    ...
                                    @if (isset($order['transaction_ref']) &&
                                    ...
                                    @if (getWebConfig(name: 'order_verification'))
                                        <span>
                                            ...
                                            <strong>{{ $order['verification_code'] }}</strong>
                                        </span>
                                    @endif
                                    ...
                                    @foreach ($order->details as $key => $detail)
                                        ...
                                        @if ($productDetails)
                                            <tr>
                                                ...
                                                <td>{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $detail['price'] * $detail['qty']), currencyCode: getCurrencyCode()) }}
                                                </td>
                                                <td>
                            ...
                            <div class="col-md-9 col-lg-8">
                                <dl class="row gy-1 text-sm-right">
                                    ...
                                    <dt class="col-5">
                                        ...
                                        <br>
                                        {{ !in_array($order['coupon_code'], [0, null]) ? '(' . translate('expense_bearer_') . ($order['coupon_discount_bearer'] == 'inhouse' ? 'admin' : $order['coupon_discount_bearer']) . ')' : '' }}
                                    </dt>
                                    <dd class="col-6 title-color">
                                        -
                    ...
                    <div class="card-body text-capitalize d-flex flex-column gap-4">
                        ...
                        <div>
                            ...
                            <select name="order_status" id="order_status" class="status form-control"
                                ...
                                <option value="pending" {{ $order->order_status == 'pending' ? 'selected' : '' }}
                                    {{ in_array($order->order_status, ['confirmed', 'processing', 'pickup', 'out_for_delivery', 'delivered', 'returned', 'failed_to_deliver', 'canceled']) ? 'disabled' : '' }}>
                                    {{ translate('pending') }}
                                </option>
                                <option value="confirmed" {{ $order->order_status == 'confirmed' ? 'selected' : '' }}
                                    {{ in_array($order->order_status, ['processing', 'pickup', 'out_for_delivery', 'delivered', 'returned', 'failed_to_deliver', 'canceled']) ? 'disabled' : '' }}>
                                    {{ translate('confirmed') }}
                                </option>
                                <option value="processing" {{ $order->order_status == 'processing' ? 'selected' : '' }}
                                    {{ in_array($order->order_status, ['pickup', 'out_for_delivery', 'delivered', 'returned', 'failed_to_deliver', 'canceled']) ? 'disabled' : '' }}>
                                    {{ translate('packaging') }}
                                </option>
                                ...
                                @if ($order['delivery_type'] == 'self_delivery')
                                    <option value="out_for_delivery"
                                        ...
                                        {{ in_array($order->order_status, ['delivered', 'returned', 'failed_to_deliver', 'canceled']) ? 'disabled' : '' }}>
                                        {{ translate('out_for_delivery') }}
                                    </option>
                                    <option value="delivered" {{ $order->order_status == 'delivered' ? 'selected' : '' }}
                                        {{ in_array($order->order_status, ['returned', 'failed_to_deliver', 'canceled']) ? 'disabled' : '' }}>
                                        {{ translate('delivered') }}
                                    </option>
                                    <option value="returned" {{ $order->order_status == 'returned' ? 'selected' : '' }}
                                        {{ in_array($order->order_status, ['failed_to_deliver', 'canceled']) ? 'disabled' : '' }}>
                                        {{ translate('returned') }}
                                    </option>
                                    <option value="failed" {{ $order->order_status == 'failed' ? 'selected' : '' }}
                                        {{ in_array($order->order_status, ['canceled']) ? 'disabled' : '' }}>
                                ...
                                @else
                                    <option value="pickup" {{ $order->order_status == 'pickup' ? 'selected' : '' }}
                                        {{ in_array($order->order_status, ['delivered', 'returned', 'failed_to_deliver', 'canceled']) ? 'disabled' : '' }}>
                                        {{ translate('pick_up') }}
                                    </option>
                                    @if ($order->order_status == 'pickup' || $order->order_status == 'out_for_delivery')
                                        <option value="out_for_delivery"
                                            {{ $order->order_status == 'out_for_delivery' ? 'selected' : '' }}
                                            {{ in_array($order->order_status, ['delivered', 'returned', 'failed_to_deliver', 'canceled']) ? 'disabled' : '' }}>
                                            {{ translate('out_for_delivery') }}
                        ...
                        <div class="d-flex justify-content-between align-items-center gap-10 form-control flex-wrap h-100">
                            ...
                            <div class="d-flex justify-content-end min-w-100 align-items-center gap-2">
                                ...
                                <label
                                    class="switcher payment-status-text {{ $order->payment_method == 'cash_on_delivery' && $order['order_status'] != 'delivered' ? 'payment-status-alert' : ($order['payment_status'] == 'paid' ? 'payment-status-alert' : '') }}">
                                    ...
                                    <div class="p-2 bg-light rounded mt-4">
                                        <div class="media m-1 gap-3">
                                            ...
                                            <div class="media-body">
                                                ...
                                                <span class="fz-12 title-color">{{ translate('track_ID') }} :
                                                    {{ $order->third_party_delivery_tracking_id }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                            ...
                            @if ($order['delivery_type'] == 'self_delivery')
                                ...
                                <li id="choose_delivery_man" class="mt-3 choose_delivery_man">
                                    ...
                                    @if (isset($order->deliveryMan))
                                        <div class="p-2 bg-light rounded mt-4">
                                            <div class="media m-1 gap-3">
                                                <img class="avatar rounded-circle"
                                                    src="{{ getvalidImage('storage/app/public/delivery-man/' . ($order->deliveryMan->image ?? ''), 'backend-profile') }}"
                                                    alt="Image">
                                                <div class="media-body">
                                                    <h5 class="mb-1">
                                        ...
                                        <div class="p-2 bg-light rounded mt-4">
                                            <div class="media m-1 gap-3">
                                                <img class="avatar rounded-circle"
                                                    ...
                                                    alt="{{ translate('image') }}">
                                                <div class="media-body">
                                                    <h5 class="mt-3">{{ translate('no_delivery_man_assigned') }}
                                                    </h5>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                @if (isset($order->deliveryMan))
                                    <li class="choose_delivery_man mt-3">
                                        <label class="font-weight-bold title-color d-flex fz-14">
                                            ...
                                            <span class="input-label-secondary cursor-pointer" data-toggle="tooltip"
                                                ...
                                                <img width="16"
                                                    src="{{ dynamicAsset('public/assets/back-end/img/info-circle.svg') }}"
                                                    alt="">
                                            </span>
                                        </label>
                                    ...
                                    <li class="choose_delivery_man mt-3">
                                        ...
                                        <input type="date" value="{{ $order->expected_delivery_date }}"
                                            data-order-id="{{ $order['id'] }}" name="expected_delivery_date"
                                            id="expected_delivery_date" class="form-control deliveryDateUpdate" required>
                                    </li>
                                @endif
                            @elseif ($order['delivery_type'] == 'shipway')
                                <li class="mt-3" id="">
                                    <div class="p-2 bg-light rounded mt-4">
                                        <img class="avatar rounded-circle"
                                        ...
                                        <div class="media m-1 gap-3">
                                            <div class="media-body">
                                                <div class="d-flex">
                                                    <span>{{ translate('Weight') }}:
                                                        <strong>{{ $order['order_weight'] }}</strong></span>
                                                    <span>{{ translate('Length') }}:
                                                        <strong>{{ $order['box_length'] }}</strong></span>
                                                    <span>{{ translate('Breath') }}:
                                                        <strong>{{ $order['box_breadth'] }}</strong></span>
                                                    <span>{{ translate('Height') }}:
                                                        <strong>{{ $order['box_height'] }}</strong></span>
                                                </div>
                                                <span class="fz-12 title-color">{{ translate('service_name') }}:
                                                    <b>{{ $order->delivery_service_name ?? translate('not_assign_yet') }}</b></span>
                                                <span class="fz-12 title-color">{{ translate('Delivery_charge') }}:
                                                    <b>Rs.
                                                        {{ $order->delivery_charge ?? translate('not_assign_yet') }}/-</b></span><br>
                                                <span class="fz-12 title-color">{{ translate('AWB') }}:
                                                    <b>{{ $order->third_party_delivery_tracking_id ?? translate('Waiting_for_tracking_number') }}</b></span><br>
                                                <span class="fz-12 title-color">{{ translate('Manifest_ID') }}:
                                                    <b>{{ $order->manifest_id ?? translate('Waiting_for_manifest_id') }}</b></span><br>
                                                <span class="fz-12 title-color">{{ translate('Shipway_Message') }}:
                                                    <b>{{ $order->message ?? translate('Shipway Message') }}</b></span>
                                            </div>
                                        </div>
                                        @if ($order->third_party_delivery_tracking_id)
                                            <div class="d-flex">
                                                <a href="{{ $order->shippingurl ?? '#' }}" target="_blank"
                                                    class="btn btn-info btn-sm mr-2" title="Invoice Download"><i
                                                        class="tio-download-to"></i></a>
                                                <form id="delivery-cancel-form" action="{{ route('vendor.orders.delivery.shipmentcancel') }}"  method="POST">
                                                    @csrf
                                                    <input type="hidden" name="delivery_partner"
                                                        value="{{ $order->delivery_partner }}">
                                                    <input type="hidden" name="third_party_delivery_tracking_id"
                                                        value="{{ $order->third_party_delivery_tracking_id }}">
                                                    <input type="hidden" name="delivery_order_id"
                                                        value="{{ $order['delivery_order_id'] }}">
                                                    <input type="hidden" name="delivery_channel_id"
                                                        value="{{ $order->delivery_channel_id }}">
                                                    <button type="button" id="cancel-shipment-btn" class="btn btn-danger btn-sm" title="Shipment Cancel">  <i class="tio-truck"></i>
                                                        </button>
                                                </form>
                                            </div>
                                        @endif
                            ...
                            @if (!empty($order->seller->shop))
                                ...
                                <div class="media-body d-flex flex-column gap-2">
                                    ...
                                    <div class="d-flex align-items-start gap-2">
                                        ...
                                        {{ $order->seller->warehouse_id }}
                                    </div>
                                    <div class="d-flex align-items-start gap-2">
                                        <img src="{{ dynamicAsset(path: 'public/assets/back-end/img/location.png') }}"
                                            class="mt-1" alt="">
                                        {{ $order->seller->shop->address }} {{ $order->seller->shop->pincode }}
                                    </div>
                                </div>
                            @else
                                <div class="card-body">
    ...
    @if ($order['order_status'] != 'delivered')
        <div class="modal fade" id="shippingAddressUpdateModal" tabindex="-1"
            aria-labelledby="shippingAddressUpdateModal" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header pb-4">
                                            ...
                                            <select name="country" id="country" class="form-control">
                                                @forelse($countries as $country)
                                                    <option value="{{ $country['name'] }}"
                                                        {{ isset($shippingAddress) && $country['name'] == $shippingAddress->country ? 'selected' : '' }}>
                                                        {{ $country['name'] }}</option>
                                                @empty
                                            ...
                                            @if ($zipRestrictStatus == 1)
                                                <select name="zip" class="form-control" data-live-search="true"
                                                    ...
                                                    @forelse($zipCodes as $code)
                                                        <option
                                                            value="{{ $code->zipcode }}"{{ isset($shippingAddress) && $code->zipcode == $shippingAddress->zip ? 'selected' : '' }}>
                                                            {{ $code->zipcode }}</option>
                                                    @empty
                                    ...
                                    <div class="d-flex flex-column align-items-center gap-2">
                                        ...
                                        <div class="row gx-2">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    <select name="country" id="country" class="form-control">
                                                        @forelse($countries as $country)
                                                            <option value="{{ $country['name'] }}"
                                                                {{ isset($billing) && $country['name'] == $billing->country ? 'selected' : '' }}>
                                                                {{ $country['name'] }}</option>
                                                        @empty
                                                            <option value="">
                                                                {{ translate('No_country_to_deliver') }}</option>
                                                        @endforelse
                                                    </select>
                                            ...
                                            <div class="col-md-3">
                                                <div class="form-group">
                                                    <label for="city"
                                                        class="title-color">{{ translate('city') }}</label>
                                                    <input type="text" name="city" id="city"
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    @if ($zipRestrictStatus == 1)
                                                        <select name="zip" class="form-control"
                                                            ...
                                                            @forelse($zipCodes as $code)
                                                                <option
                                                                    value="{{ $code->zipcode }}"{{ isset($billing) && $code->zipcode == $billing->zip ? 'selected' : '' }}>
                                                                    {{ $code->zipcode }}</option>
                                                            @empty
                                                                <option value="">
                                                                    {{ translate('no_zip_to_deliver') }}</option>
                                                            @endforelse
                                                        </select>
                                                    @else
                                                        <input type="text" class="form-control"
                                            ...
                                            <input type="hidden" name="longitude" class="form-control"
                                                placeholder="{{ translate('ex') . ' ' . ':' . ' ' . '103.344322' }}"
                                                id="billing_longitude" value="{{ $billing->longitude ?? 0 }}" required
                                                readonly>
                                            <div class="col-12 ">
    ...
    {{-- Shipway Order Assign the order  --}}
    <div class="modal fade" id="packaging" tabindex="-1" aria-labelledby="packagingLabel" aria-hidden="true"
        data-keyboard="false" data-backdrop="static">
        <div class="modal-dialog modal-">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="packagingLabel">Fill the Order Detail for Shipment</h5>
                    <span class="input-label-secondary cursor-pointer" data-toggle="tooltip" data-placement="right"
                        title="{{ translate('please_this_button_click_show_the_processing_do_not') . ' ' . translate('refresh_page_and_don`t_close_the_modal') . '.' }}">
                        <img width="16" src="{{ dynamicAsset('public/assets/back-end/img/info-circle.svg') }}"
                            alt="">
                    </span>
                    <br>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                ...
                <div class="modal-body">
                    <form id="delivery-partner-form" action="#" method="post">
                        @csrf
                        <input type="hidden" id="frompincode" name="fromPincode"
                            value="{{ $order->seller->shop->pincode }}" class="form-control">
                        <input type="hidden" id="topincode" name="toPincode"
                            value="{{ isset($shippingAddress->zip) ? $shippingAddress->zip : $billing->zip ?? '' }}"
                            class="form-control">
                        <input type="hidden" id="paymenttype" name="paymentType"
                            value="{{ $order['payment_method'] == 'razor_pay' ? 'prepaid' : ($order['payment_method'] == 'cash_on_delivery' ? 'cod' : ($order['payment_method'] == 'pay_by_wallet' ? 'prepaid' : '')) }}"
                            class="form-control">
                        <input type="hidden" id="id" name="id" value="{{ $order['id'] }}"
                            class="form-control">
                        <input type="hidden" name="warehouse_id" id="warehouseid"
                            value="{{ $order->seller->warehouse_id }}" class="form-control">
                        <input type="hidden" name="order_status" value="processing" class="form-control">
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label class="title-color">{{ translate('order_weight_(Grams 0.0)') }}</label>
                                <input type="text" id="weight" name="order_weight" class="form-control w-100"
                                    placeholder="order weight(Grams) 0.0" required
                                    value="{{ $order['order_weight'] ?? '' }}">
                            </div>
                            <div class="form-group col-md-6">
                                <label class="title-color">{{ translate('box_length_(Cms)') }}</label>
                                <input type="text" id="length" name="box_length" class="form-control w-100"
                                    placeholder="box length(CMS)" required value="{{ $order['box_length'] ?? '' }}">
                            </div>

                            <div class="form-group col-md-6">
                                <label class="title-color">{{ translate('box_breadth_(Cms)') }}</label>
                                <input type="text" id="breadth" name="box_breadth" class="form-control w-100"
                                    placeholder="box breadth(CMS)" required value="{{ $order['box_breadth'] ?? '' }}">
                            </div>
                            <div class="form-group col-md-6">
                                <label class="title-color">{{ translate('box_height_(Cms)') }}</label>
                                <input type="text" id="height" name="box_height" class="form-control w-100"
                                    placeholder="box height(CMS)" required value="{{ $order['box_height'] ?? '' }}">
                            </div>

                        </div>
                        <div class="row justify-content-end gap-3 mt-3 mx-1">
                            <button type="button" class="btn btn--primary px-4" id="orderUpdate"
                                onclick="orderDetails()">{{ translate('order_pickup') }}</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    {{-- Shipway Order Assign the order  --}}
    {{-- ShipWay Orrder Pickup the order  --}}
    <div class="modal fade" id="pickup" tabindex="-1" aria-labelledby="pickupLabel" aria-hidden="true"
        data-keyboard="false" data-backdrop="static">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="pickupLabel">Order Pickup Details</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="delivery-pickup-form" action="#" method="post">
                        @csrf
                        <input type="hidden" id="id" name="id" value="{{ $order['id'] }}"
                            class="form-control">
                        <input type="hidden" name="order_status" value="pickup" class="form-control">
                        <input type="hidden" name="warehouse_id" id="warehouseid"
                            value="{{ $order->seller->warehouse_id }}" class="form-control">
                        <input type="hidden" name="return_warehouse_id" id="warehouseidr"
                            value="{{ $order->seller->warehouse_id }}" class="form-control">
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label class="title-color">{{ translate('pickup_date') }}</label>
                                <input type="date" id="pickupdate" name="pickup_date" class="form-control w-100"
                                    placeholder="pickup date" required>
                            </div>
                            <div class="form-group col-md-6">
                                <label class="title-color">{{ translate('pickup_time') }}</label>
                                <input type="time" id="pickuptime" name="pickup_time" class="form-control w-100"
                                    placeholder="pickup time" required>
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label class="title-color">{{ translate('office_close_time') }}</label>
                                <input type="time" id="officetime" name="office_close_time"
                                    class="form-control w-100" placeholder="office close time" required>
                            </div>
                            <div class="form-group col-md-6">
                                <label class="title-color">{{ translate('package_count') }}</label>
                                <input type="number" id="packagecouunt" name="package_count" class="form-control w-100"
                                    placeholder="package count" min="1" required>
                            </div>
                        </div>
                        <div class="row justify-content-end gap-3 mt-3 mx-1">
                            <button type="button" class="btn btn--primary px-4" id="pickupOrder"
                                onclick="orderPickup()">{{ translate('order_pickup') }}</button>
                        </div>
                    </form>
                </div>
            </div>
...
@push('script')
    ...
    <script>
        document.getElementById('cancel-shipment-btn').addEventListener('click', function (e) {
            Swal.fire({
                title: 'Are you sure?',
