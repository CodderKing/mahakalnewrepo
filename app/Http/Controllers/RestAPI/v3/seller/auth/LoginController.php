...
class LoginController extends Controller
{
    public function login(Request $request)
    {
        ...
        if (isset($seller) && ($seller['status'] == 'approved' || $seller['status'] == 'hold' || $seller['status'] == 'pending') && auth('seller')->attempt($data)) {
            // $token = Str::random(50);
            ...
            } else if ($seller['type'] == 'trast') {
            }

            return response()->json(['token' => $token, 'type' => auth('seller')->user()->type, 'id' => auth('seller')->user()->relation_id, 'status' => auth('seller')->user()->status], 200);
        } else {
            $errors = [];
