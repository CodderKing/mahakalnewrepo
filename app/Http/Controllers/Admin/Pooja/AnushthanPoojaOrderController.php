...
class AnushthanPoojaOrderController extends Controller
{
    use PdfGenerator;
    ...
    public function orders_list($status, Request $request)
    {
        ...
        Service_order::where('type', 'anushthan')->where('package_id', 7)->where('status', 0)->where('order_status', 0)
        ...
        if ($status == 'all') {
            $orders = Service_order::where('type', 'anushthan')->with('leads')->with('packages')->with('vippoojas')->with('customers')->with('pandit')->orderBy('created_at', 'DESC')->paginate(10);
        } else {
            $orders = Service_order::where('type', 'anushthan')->where('status', $status)->with('leads')->with('customers')->with('vippoojas')->with('pandit')->orderBy('created_at', 'DESC')->paginate(10);
        }
        $users = User::all();
        ...
        return view('admin-views.pooja.anushthanorder.list', compact('orders', 'users', 'payment_gateways_list', 'paymentGatewayPublishedStatus', 'digital_payment'));
    }

    public function AnushthanData(Request $request): JsonResponse
    {
    ...
    {
        $orders = Service_order::where('type', 'anushthan')->where('status', 0)->where('is_completed', 0)->where('package_id', 8)->where('payment_status', 1)
            ->with(['packages', 'vippoojas', 'customers', 'pandit'])->orderBy('created_at', 'DESC')->paginate(10);
        return view('admin-views.pooja.anushthanorder.listinstanceorder', compact('orders'));
    }

    public function checked_order()
    {
        $anushthan = Service_order::where('checked', 0)->update(['checked' => 1]);
        if ($anushthan) {
    ...
    public function orders_details($id)
    {
        ...
        $inHouseAstrologers = Astrologer::select('id', 'name', 'is_pandit_pooja_per_day')
            ...
            ->where('type', 'in house')
            ->where('status', 1)
            ->whereRaw("JSON_CONTAINS_PATH(is_pandit_anushthan, 'one', '$.\"$serviceId\"')")
            ->get();
        $freelancerAstrologers = Astrologer::select('id', 'name', 'is_pandit_pooja_per_day', 'is_pandit_anushthan')
            ->where('primary_skills', 3)->where('type', 'freelancer')->where('status', 1)
            ->whereRaw("JSON_CONTAINS_PATH(is_pandit_anushthan, 'one', '$.\"$serviceId\"')")->get()
            ->map(function ($astrologer) use ($serviceId) {
                $isPanditPooja = json_decode($astrologer->is_pandit_anushthan, true);
        ...
        $details['pooja_pandit'] = Astrologer::where('primary_skills', '3')->where('is_pandit_anushthan', 'like', '%' . $details['service_id'] . '%')->where('status', 1)->get();
        return view('admin-views.pooja.anushthanorder.details', compact('details', 'freelancerAstrologers', 'inHouseAstrologers'));
    }
    ...
    public function orders_generate_invoice($id)
    {
        ...
        // dd($details);
        $mpdf_view = PdfView::make('admin-views.pooja.anushthanorder.invoice', compact('details', 'companyPhone', 'companyEmail', 'companyName', 'companyWebLogo', 'companyAddress', 'companygst', 'companypan'));
    ...
    public function orders_assign_pandit(Request $request){
        ...
        // Step 1: Fetch Orders Matching the Criteria
           $orders = Service_order::where('type', 'anushthan')
           ...
           if (!$getPanditData || !$getPanditData->is_pandit_anushthan) {
               Toastr::error(translate('Pandit or price data not found'));
               return back();
           }
   
           // Step 3: Decode price list
           $priceList = json_decode($getPanditData->is_pandit_anushthan, true);
           $price = isset($priceList[$request->service_id]) ? $priceList[$request->service_id] : 0;
           $commissionList = json_decode($getPanditData->is_pandit_anushthan_commission, true);
           ...
           foreach ($orders as $order) {
               ...
               $existingTransaction = PanditTransectionPooja::where('service_id', $order->service_id)
                   ->where('booking_date', $order->booking_date)->where('type','vip')
    ...
    {
        // dd($request->all());
        $orders = Service_order::where('id', $id)->where('type', 'anushthan')->where('service_id', $request->service_id)->where('payment_status', 1)->where('booking_date', $request->booking_date)->where('package_id', $request->package_id)->with(['vippoojas', 'customers', 'payments'])->first();
        if (!$orders) {
        ...
        if ($orders->customers) {
            ...
            if ($userInfo && !empty($userInfo['email']) && filter_var($userInfo['email'], FILTER_VALIDATE_EMAIL)) {
                $service_name = \App\Models\Vippooja::where('id', $orders->service_id)
                    ->where('is_anushthan', 1)->first();
                $bookingDetails = \App\Models\Service_order::where('service_id', ($orders->service_id ?? ""))
                    ->where('type', 'anushthan')->where('booking_date', ($orders->booking_date ?? ""))->where('customer_id', ($orders->customer_id ?? ""))->where('order_id', ($orders->order_id ?? ""))->first();
                $data = [
                    'type' => 'pooja',
    ...
    public function status_times($id, Request $request)
    {
        ...
        $scheduledTime = !empty($request->schedule_time) && strtotime($request->schedule_time)
            ? Carbon::parse($request->schedule_time)->format('h:i A')
            : 'N/A';
        if ($orders->customers) {
            $messageData = [
                'service_name' => $service->name,
            ...
            if ($userInfo && !empty($userInfo['email']) && filter_var($userInfo['email'], FILTER_VALIDATE_EMAIL)) {
                ...
                $data = [
                    'type' => 'pooja',
                    'email' => $userInfo->email,
                    'subject' => 'Anusthan Time Scheduled',
    ...
    public function cancel_poojas($id, Request $request)
    {
        ...
        return redirect()->route('admin.anushthan.order.list', ['2']);
    }
    //----------------------------------------Order by Pooja Change The option ------------------------------
    //All ORDER PENDIT ASSING
    public function all_orders_assign_pandit(Request $request)
    {
        $pandit = Service_order::where('type', 'anushthan')->where('service_id', $request->service_id)->where('booking_date', $request->booking_date)->update(['pandit_assign' => $request->pandit_id]);
        // dd($pandit);
        if ($pandit) {
    ...
    {
        $orders = Service_order::where('type', 'anushthan')->where('service_id', $request->service_id)->where('booking_date', $request->booking_date)->where('package_id', 7)->with(['vippoojas', 'customers', 'payments'])->get();
        if (!$orders) {
            Toastr::error(translate('order_not_found'));
            return back();
        }
        ...
        foreach ($orders as $pooja) {
            if ($request->order_status == 1) {
                ...
                Service_order::where('service_id', $request->service_id)->update([
                    'status' => $request->order_status,
                    'is_completed' => $request->order_status,
                    'is_edited' => $request->order_status,
                    'order_status' => $request->order_status,
            ...
            if ($pooja->customers) {
                ...
                if ($userInfo && !empty($userInfo['email']) && filter_var($userInfo['email'], FILTER_VALIDATE_EMAIL)) {
                    ...
                    $bookingDetails = \App\Models\Service_order::where('service_id', ($pooja->service_id ?? ""))
                        ->where('type', 'anushthan')->where('booking_date', ($pooja->booking_date ?? ""))->where('customer_id', ($pooja->customer_id ?? ""))->where('order_id', ($pooja->order_id ?? ""))->first();

                    $data = [
                        'type' => 'pooja',
                        'email' => $userInfo->email,
                        'subject' => 'Anusthan Completed.',
                        'htmlContent' => \Illuminate\Support\Facades\View::make('admin-views.email.email-template.pooja-complete', compact('userInfo', 'service_name', 'bookingDetails'))->render(),
                    ];
                    SendEmailPoojaJob::dispatch($data);
                }
    ...
    public function status_time(Request $request)
    {
        ...
        $orders = Service_order::where('type', 'anushthan')->where('status', 0)->where('package_id', $request->package_id)->where('booking_date', $request->booking_date)->where('service_id', $request->service_id)->with(['vippoojas', 'customers'])->get();
        foreach ($orders as $pooja) {
    ...
    public function pooja_video(Request $request)
    {
        ...
        // dd($orders);
        Toastr::success(translate('Anushthan_video_changed_successfully'));
    ...
    public function cancel_pooja(Request $request)
    {
        ...
        // dd($orders);
        Toastr::success(translate('Anushthan_Cancel_successfully'));
        return redirect()->route('admin.anushthan.order.list', ['2']);
    }
    //----------------------------------------Order by Pooja Change The option ------------------------------
    ...
    public function lead_list(Request $request)
    {
        if ($request->has('searchValue')) {
            $leads = Leads::where('person_name', 'like', '%' . $request->searchValue . '%')->where('status', 1)->where('type', 'anushthan')->with('vippooja')->orderBy('created_at', 'DESC')->paginate(10);
        } else {
            $leads = Leads::where('status', 1)->where('type', 'anushthan')->with('vippooja', 'followby')->orderBy('created_at', 'DESC')->paginate(10);
        }
        // dd($leads);
        return view('admin-views.pooja.anushthanlead.list', compact('leads'));
    }
    ...
    // Order By Pooja
    public function orders_by_anushthan(Request $request)
    {
        
        // Fetch orders with package_id 7
        $viporders = Service_order::where('type', 'anushthan')
            ->where('status', 0)
            ->where('is_completed', 0)
            ->where('package_id', 7)->where('payment_status', 1)
            ->with(['packages', 'vippoojas', 'customers', 'pandit'])
            ->selectRaw('service_id, COUNT(*) as total_orders, pandit_assign, booking_date, COUNT(created_at) as booking_count, SUM(pay_amount) as total_amount, GROUP_CONCAT(members SEPARATOR "|") as members, order_status, package_id')
            ->groupBy('service_id', 'booking_date', 'pandit_assign')
            ->orderBy('total_orders', 'DESC')
            ->get();

        // Fetch orders with package_id 8
        $instanceorders = Service_order::where('type', 'anushthan')
            ->where('status', 0)
            ->where('is_completed', 0)
            ->where('package_id', 8)->where('payment_status', 1)
            ->with(['packages', 'vippoojas', 'customers', 'pandit'])
            ->selectRaw('service_id, COUNT(*) as total_orders, pandit_assign, booking_date, COUNT(created_at) as booking_count, SUM(pay_amount) as total_amount, GROUP_CONCAT(members SEPARATOR "|") as members, order_status,package_id')
            ->groupBy('service_id', 'booking_date', 'pandit_assign')
            ->orderBy('total_orders', 'DESC')
        ...
        return view('admin-views.pooja.anushthanorder.orderbyanushthanpooja', compact('viporders', 'instanceorders', 'users'));
    }
    // All Single Order
    function all_single_order($service_id, $booking_date, $status)
    {
        ...
        $orders = Service_order::where('type', 'anushthan')->where('service_id', $service_id)->where('booking_date', $booking_date)->where('order_status', $status)->with(['vippoojas'])->get();
        // dd($orders);
        $pending = Service_order::where('type', 'anushthan')->where('service_id', $service_id)->where('booking_date', $booking_date)->where('order_status', $status)->first();
        if (!$pending) {
            $pending = null;
        }
        // dd($pending);
        return view('admin-views.pooja.anushthanorder.SingleOrder', compact('orders', 'service', 'pending'));
    }
    ...
    public function single_orders_details($booking_date, $service_id, $status)
    {
        ...
        return view('admin-views.pooja.anushthanorder.SingleOrderdetails', compact('details', 'service', 'inHouseAstrologers'));
    }
    ...
    public function instance_orders_details($booking_date, $service_id, $status)
    {
        ...
        $query = Service_order::where('type', 'anushthan')->where('service_id', $service_id)->where('booking_date', $booking_date)->where('status', $status)->where('package_id', 8)->with(['vippoojas', 'packages', 'pandit'])
            ->selectRaw('service_id, COUNT(*) as total_orders, pandit_assign, booking_date, COUNT(booking_date) as booking_count, SUM(pay_amount) as total_amount, 
                GROUP_CONCAT(COALESCE(members, "") SEPARATOR "|") as members, 
                GROUP_CONCAT(COALESCE(gotra, "") SEPARATOR "|") as gotra, 
                GROUP_CONCAT(COALESCE(leads_id, "") SEPARATOR ",") as leads, 
        ...
        // Check status condition
        $details = ($status == '1') ? $query->first() : $query->first();
        return view('admin-views.pooja.anushthanorder.instanceOrderdetails', compact('details', 'service', 'inHouseAstrologers'));
    }
    ...
    public function all_instance_status(Request $request)
    {
        ...
        foreach ($orders as $pooja) {
            if ($request->order_status == 1) {
                ...
                Service_order::where('order_id', $pooja['order_id'])->where('package_id', 8)->update([
                    'status' => $request->order_status,
                    'is_completed' => $request->order_status,
                    'is_edited' => $request->order_status,
                    'order_status' => $request->order_status,
                ...
                $status = Service_order::where('order_id', $pooja['order_id'])->where('package_id', 8)->update([
                    'order_status' => $request->order_status,
                ]);
            } else {
                $status = Service_order::where('order_id', $pooja['order_id'])->where('package_id', 8)->update([
            ...
            if ($pooja->customers) {
                $messageData = [
                    ...
                    'share_video' => $request->pooja_video ?? 'mahakal.com',
                    'customer_id' => $pooja->customer_id,
                    'pooja' => 'Anushthan Instance Pooja',
                    'attachment' => asset('public/' . $certificatePath),
                    'type' => 'text-with-media',
                ];
                ...
                if ($userInfo && !empty($userInfo['email']) && filter_var($userInfo['email'], FILTER_VALIDATE_EMAIL)) {
                    ...
                    $data = [
                        'type' => 'pooja',
                        'email' => $userInfo->email,
                        'subject' => 'Anushthan Successfully completed.',
                        'htmlContent' => \Illuminate\Support\Facades\View::make('admin-views.email.email-template.pooja-complete', compact('userInfo', 'service_name', 'bookingDetails'))->render(),
                    ];
                    SendEmailPoojaJob::dispatch($data);
                }
        ...
        Toastr::success(translate('status_changed_successfully'));
        return redirect()->route('admin.anushthan.order.list', ['status' => 1]);
    }
    ...
    public function cancel_instance($service_id, $status, Request $request)
    {
        // dd($request->all());
        $orders = Service_order::where('type', 'anushthan')->where('package_id', $request->package_id)->where('booking_date', $request->booking_date)->where('service_id', $request->service_id)->get();
        foreach ($orders as $pooja) {
        ...
        return redirect()->route('admin.anushthan.orders.list', ['2']);
    }
    public function order_rejected_update(Request $request)
    {
        ...
        $service_name = \App\Models\Vippooja::where('id', ($whatsappData->service_id ?? ""))->where('is_anushthan', 1)->first();
        ...
        if ($userInfo && !empty($userInfo['email']) && filter_var($userInfo['email'], FILTER_VALIDATE_EMAIL)) {
            ...
            $data['subject'] = 'Reschedule for Your Anushthan  Services';
        ...
        if ($updated) {
            Toastr::success(translate('Anushthan_Reschedule_Successfully'));
        } else {
            Toastr::error(translate('Anushthan_Reschedule_Unsuccessfully'));
        }
        return redirect()->route('admin.anushthan.order.list', ['6']);
    }

    public function orders_getpandit($serviceId, $bookingDate)
    {
        // Fetch in-house astrologers
        $inHouseAstrologers = Astrologer::select('id', 'name', 'is_pandit_pooja_per_day', 'type', 'is_pandit_anushthan')
        ...
        $freelancerAstrologers = Astrologer::select('id', 'name', 'is_pandit_pooja_per_day', 'type', 'is_pandit_anushthan')
            ->where('primary_skills', 3)
            ->where('type', 'freelancer')
            ->where('status', 1)->whereRaw("JSON_CONTAINS_PATH(is_pandit_anushthan, 'one', '$.\"{$serviceId}\"')")->get()
    ...
    {
        $orders = Service_order::where('type', 'anushthan')->where('service_id', $service_id)->where('booking_date', $booking_date)->where('order_status', $status)->get();
        $vipooja_name = Vippooja::where('id', $service_id)->value('name');
        $bookingDate = optional($orders->first())->booking_date;
        $mpdf_view = PdfView::make('admin-views.pooja.anushthanorder.member-list', compact('orders', 'vipooja_name', 'bookingDate'));
    ...
    public function send_whatsapp_leads($id)
    {
        ...
        if ($lead) {
            $message_data = [
                ...
                'puja' => 'Anushthan',
                'link' => 'mahakal.com/anushthan/anushthan/' . $poojaName->slug,
    ...
    {
        $orders = Service_order::where('customer_id', $request->customer_id)->where('type','anushthan')->where('is_block', '!=', 9)
            ->select('order_id')
            ->get();
        return response()->json([
            'orders' => $orders
