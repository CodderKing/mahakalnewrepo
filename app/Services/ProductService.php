...
class ProductService
{
    ...
    public function getSkuCombinationView(object $request): string
    {
        ...
        return view(Product::SKU_COMBINATION[VIEW], compact('combinations', 'unitPrice', 'colorsActive', 'productName', 'request'))->render();
    }
    ...
    public function getAddProductData(object $request, string $addedBy): array
    {
        ...
        return [
            ...
            // 'forward_price' => $request['forward_price'],
            // 'reverse_price' => $request['reverse_price'],
            'product_refund' => isset($request['product_refund']) && $request['product_refund'] == '1' ? 1 : 0,
            ...
            'current_stock' => $request['product_type'] == 'physical' ? abs($stockCount) : 0,
            ...
            'thumbnail' => $this->upload(dir: 'product/thumbnail/', format: 'webp', image: $request['image']),
            'digital_file_ready' => $digitalFile,
            'meta_title' => $request['meta_title'],
            'meta_description' => $request['meta_description'],
            'meta_image' => $this->upload(dir: 'product/meta/', format: 'png', image: $request['meta_image']),
        ];
    }
    ...
    public function getUpdateProductData(object $request, object $product, string $updateBy): array
    {
        ...
        if ($product->unit_price != $dataArray['unit_price']) {

            \DB::table('carts')
                ->where('product_id', $product->id)
                ->update(['price' => $dataArray['unit_price']]);
        }
