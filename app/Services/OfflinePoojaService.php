...
class OfflinePoojaService
{
    ...
    public function getAddOfflinePoojaData(object $request, string $addedBy): array
    {   
        
        $processedImages = $this->getProcessedImages(request: $request);
        $digitalFile = '';
        if ($request['digital_product_type'] == 'ready_product') {
            $digitalFile = $this->fileUpload(dir: 'offlinepooja/digital-product/', format: $request['digital_file_ready']->getClientOriginalExtension(), file: $request['digital_file_ready']);
        }
        $packageData=[];
        $package=$request['package_details'];
        foreach($package as $key=> $pac){
            $packageData[$key]=['package_id'=>$pac,'price'=>$request['price'][$key],'percent'=>$request['percent'][$key]];
        }
        ...
        return [
            ...
            'package_details' => json_encode($packageData),
            'type' => $request['type'],
            ...
            'terms_conditions' => $request['terms_conditions'][array_search('en', $request['lang'])],
            'video_url' => $request['video_url'],
            'status' => 1,
            'images' => json_encode($processedImages['image_names']),
            'thumbnail' => $this->upload(dir: 'offlinepooja/thumbnail/', format: 'png', image: $request['thumbnail']),
            // 'digital_file_ready' => $digitalFile,
            'meta_title' => $request['meta_title'],
            'meta_description' => $request['meta_description'],
            'meta_image' => $this->upload(dir: 'offlinepooja/meta/', format: 'png', image: $request['meta_image']),
        ];
    }
    // Update Service GET
    ...
    {
        // dd($request->all());
        $processedImages = $this->getProcessedUpdateImages(request: $request, offlinePooja: $offlinePooja);
        $digitalFile = null;
        if ($request['product_type'] == 'offlinepooja') {
            if ($request['digital_product_type'] == 'ready_product' && $request->hasFile('digital_file_ready')) {
                $digitalFile = $this->update(dir: 'product/digital-product/', oldImage: $offlinePooja['digital_file_ready'], format: $request['digital_file_ready']->getClientOriginalExtension(), image: $request['digital_file_ready'], fileType: 'file');
            } elseif (($request['digital_product_type'] == 'ready_after_sell') && $offlinePooja['digital_file_ready']) {
                $this->delete(filePath: 'offlinepooja/digital-product/' . $offlinePooja['digital_file_ready']);
            }
        } elseif ($request['product_type'] == 'pooja' && $offlinePooja['digital_file_ready']) {
            $this->delete(filePath: 'offlinepooja/digital-product/' . $offlinePooja['digital_file_ready']);
        }
        $packageData=[];
        $package=$request['package_details'];
        ...
        $dataArray = [
            ...
            'meta_title' => $request['meta_title'],
            'meta_description' => $request['meta_description'],
            'meta_image' => $request->file('meta_image') ? $this->update(dir: 'offlinepooja/meta/', oldImage: $offlinePooja['meta_image'], format: 'png', image: $request['meta_image']) : $offlinePooja['meta_image'],
        ];

        if ($request->file('thumbnail')) {
            $dataArray += [
                'thumbnail' => $this->update(dir: 'offlinepooja/thumbnail/', oldImage: $offlinePooja['thumbnail'], format: 'webp', image: $request['thumbnail'], fileType: 'image')
            ];
        }
